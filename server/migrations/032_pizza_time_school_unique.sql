...
-- Add composite unique only if not already present (avoids "relation already exists" on restart)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'pizza_time_class_school_unique') THEN
    ALTER TABLE pizza_time ADD CONSTRAINT pizza_time_class_school_unique UNIQUE (class, school_id);
  END IF;
END $$;
