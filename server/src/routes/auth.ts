  ...
  try {
    ...
    res.json({
      ...
      impersonated_by_username: req.impersonatedBy
        ? (await database.get('SELECT username FROM users WHERE id = $1', [req.impersonatedBy]))?.username ?? null
        : null,
      allow_teacher_impersonation: req.user.role === 'teacher'
        ? (userWithJob?.school_id
            ? (await database.get('SELECT settings FROM schools WHERE id = $1', [userWithJob.school_id]))?.settings?.allow_teacher_impersonation === true
            : false)
        : false,
    });
  } catch (error) {
...
router.post('/impersonate',
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const school = await database.get('SELECT settings FROM schools WHERE id = $1 AND archived = false', [schoolId]);
      ...
      const student = await database.get(
        `SELECT * FROM users WHERE id = $1 AND role = 'student' AND school_id = $2 AND status = 'approved'`,
        [student_id, schoolId]
      );
      if (!student) {
