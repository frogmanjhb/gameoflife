...
import {
  ...
  WEEK_START_SQL,
  hasFiveMinuteLessonJob,
  isTownClass,
  isValidTiming,
  sanitizeClassContent,
  sanitizeDescription,
  sanitizeTitle,
  timingLabel,
  lessonStatusLabel,
  EventTiming,
} from '../domain/fiveMinuteLessons';

const router = Router();

async function tablesReady(): Promise<boolean> {
  try {
    await database.query('SELECT 1 FROM five_minute_lessons LIMIT 1');
    return true;
  } catch {
    return false;
  }
...
async function getOrCreateSettings(schoolId: number | null, townClass: string) {
  const existing = schoolId != null
    ? await database.get(
        'SELECT * FROM five_minute_lesson_settings WHERE school_id = $1 AND town_class = $2',
        [schoolId, townClass]
      )
    : await database.get(
        'SELECT * FROM five_minute_lesson_settings WHERE school_id IS NULL AND town_class = $1',
        [townClass]
      );
  ...
  if (schoolId != null) {
    const rows = await database.query(
      `INSERT INTO five_minute_lesson_settings (school_id, town_class, teacher_board_enabled)
       VALUES ($1, $2, true)
...
async function getStudentPref(userId: number) {
  const row = await database.get(
    'SELECT board_visible FROM five_minute_lesson_student_prefs WHERE user_id = $1',
    [userId]
  );
...
async function getWeeklySuggestionCount(userId: number): Promise<number> {
  const row = await database.get(
    `SELECT COUNT(*)::int AS count FROM five_minute_lessons
...
async function paySuggestionReward(
  ...
  if (account && SUGGESTION_EARNINGS_REWARD > 0) {
    ...
    await database.query(
      ...
      [schoolId, townClass, SUGGESTION_EARNINGS_REWARD, 'withdrawal', `5-minute lesson payout to ${username}`, userId]
    );
    await database.query(
    ...
    await database.query(
      ...
      [account.id, SUGGESTION_EARNINGS_REWARD, 'FIVE_MINUTE_LESSON_EARN']
    );
  }
...
async function buildLessonsPayload(
  schoolId: number | null,
  townClass: string,
  ...
  const lessons = schoolId != null
    ? await database.query(
        `SELECT l.*, u.username AS suggester_username,
  ...
  let votedLessonIds: number[] = [];
  if (viewerUserId != null) {
    const votes = await database.query(
      'SELECT lesson_id FROM five_minute_lesson_votes WHERE user_id = $1',
      [viewerUserId]
    );
    votedLessonIds = votes.map((v: { lesson_id: number }) => v.lesson_id);
  }
  ...
  return lessons.map((l: Record<string, unknown>) => ({
    ...
    timing: l.timing,
    timing_label: timingLabel(l.timing as EventTiming),
    status: l.status,
    status_label: lessonStatusLabel(String(l.status)),
...
function computeNeedsVote(
  boardActive: boolean,
  lessons: { status: string; has_voted: boolean }[]
): boolean {
...
router.get('/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (role === 'teacher') {
      ...
      isLessonPresenter = !!student && hasFiveMinuteLessonJob(student.job_name);
    ...
    const lessons = await buildLessonsPayload(
      townScope.schoolId,
      townScope.townClass,
      viewerUserId,
      role
    ...
    res.json({
      ...
      needs_vote: role === 'student' && computeNeedsVote(
        boardActive,
        lessons as { status: string; has_voted: boolean }[]
      ),
...
router.post('/suggest', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const student = await getStudentUser(req.user.id);
    if (!student || !hasFiveMinuteLessonJob(student.job_name)) {
    ...
    const classContent = sanitizeClassContent(req.body?.class_content);
    ...
    if (weeklyCount >= SUGGESTIONS_PER_WEEK) {
      return res.status(400).json({
        error: `You can only suggest ${SUGGESTIONS_PER_WEEK} lessons per week. Try again next week.`,
      });
    }
    ...
    const insert = townScope.schoolId != null
      ? await database.query(
          `INSERT INTO five_minute_lessons (school_id, town_class, suggested_by_user_id, title, description, class_content, timing, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7, 'pending')
           RETURNING *`,
          [townScope.schoolId, townScope.townClass, req.user.id, title, description, classContent, timing]
        )
      : await database.query(
          `INSERT INTO five_minute_lessons (school_id, town_class, suggested_by_user_id, title, description, class_content, timing, status)
           VALUES (NULL, $1, $2, $3, $4, $5, $6, 'pending')
           RETURNING *`,
          [townScope.townClass, req.user.id, title, description, classContent, timing]
        );
    ...
    res.status(201).json({
      ...
      lesson: {
        ...
        timing_label: timingLabel(lesson.timing),
        status: lesson.status,
        status_label: lessonStatusLabel(lesson.status),
...
router.post('/:id/vote', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (isNaN(lessonId)) return res.status(400).json({ error: 'Invalid lesson' });
    ...
    const lesson = townScope.schoolId != null
      ? await database.get(
          'SELECT * FROM five_minute_lessons WHERE id = $1 AND school_id = $2 AND town_class = $3',
          [lessonId, townScope.schoolId, townScope.townClass]
        )
      : await database.get(
          'SELECT * FROM five_minute_lessons WHERE id = $1 AND school_id IS NULL AND town_class = $2',
          [lessonId, townScope.townClass]
        );
    ...
    const existing = await database.get(
      'SELECT id FROM five_minute_lesson_votes WHERE lesson_id = $1 AND user_id = $2',
    ...
    await database.query(
      'INSERT INTO five_minute_lesson_votes (lesson_id, user_id) VALUES ($1, $2)',
      [lessonId, req.user.id]
    );

    const voteCountRow = await database.get(
      'SELECT COUNT(*)::int AS count FROM five_minute_lesson_votes WHERE lesson_id = $1',
      [lessonId]
    );
...
router.post('/:id/approve', authenticateToken, requireRole(['teacher']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!(await tablesReady())) {
      return res.status(503).json({ error: '5 Minute Lessons is not available yet. Please try again later.' });
    }
    ...
    const lesson = schoolId != null
      ? await database.get(
          `SELECT l.*, u.username AS suggester_username
           FROM five_minute_lessons l
           JOIN users u ON u.id = l.suggested_by_user_id
           WHERE l.id = $1 AND l.school_id = $2 AND l.town_class = $3`,
          [lessonId, schoolId, townClass]
        )
      : await database.get(
          `SELECT l.*, u.username AS suggester_username
           FROM five_minute_lessons l
           JOIN users u ON u.id = l.suggested_by_user_id
           WHERE l.id = $1 AND l.school_id IS NULL AND l.town_class = $2`,
          [lessonId, townClass]
        );
    ...
    let reward: { experience_points: number; earnings: number; new_level: number | null } | null = null;
    if (!lesson.reward_paid) {
      try {
        reward = await paySuggestionReward(
          lesson.suggested_by_user_id,
          lesson.suggester_username,
          lesson.town_class,
          lesson.school_id ?? null
        );
      } catch (err) {
...
router.post('/:id/deny', authenticateToken, requireRole(['teacher']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    if (!(await tablesReady())) {
      return res.status(503).json({ error: '5 Minute Lessons is not available yet. Please try again later.' });
    }
    ...
    await database.query(
      `UPDATE five_minute_lessons
       ...
       WHERE id = $3`,
      [req.user?.id ?? null, denialReason, lessonId]
    );
...
router.patch('/settings', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (req.user.role === 'student') {
      ...
      await database.query(
        `INSERT INTO five_minute_lesson_student_prefs (user_id, board_visible, updated_at)
    ...
    if (schoolId != null) {
      await database.query(
        `UPDATE five_minute_lesson_settings
...
router.post('/:id/close', authenticateToken, requireRole(['teacher']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    await database.query(
      `UPDATE five_minute_lessons SET status = 'closed' WHERE id = $1`,
      [lessonId]
    );
...
router.delete('/:id', authenticateToken, requireRole(['teacher']), async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const result = schoolId != null
      ? await database.run(
          'DELETE FROM five_minute_lessons WHERE id = $1 AND school_id = $2 AND town_class = $3',
          [lessonId, schoolId, townClass]
        )
      : await database.run(
          'DELETE FROM five_minute_lessons WHERE id = $1 AND school_id IS NULL AND town_class = $2',
          [lessonId, townClass]
        );
