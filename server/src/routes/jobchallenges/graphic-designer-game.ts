...
router.get('/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      await database.query('SELECT 1 FROM graphic_designer_game_sessions LIMIT 1');
    } catch (tableError) {
      return res.json({
    ...
    if (account && totalEarnings > 0) {
      ...
      if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
        ...
        await database.query(
          ...
          [schoolId, userClass, totalEarnings, 'withdrawal', `Graphic Designer Design Brief Payout to ${req.user!.username}`, userId]
        );
      }
      await database.query(`
