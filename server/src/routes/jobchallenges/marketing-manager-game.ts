...
const MARKETING_MANAGER_JOB_NAME = 'assistant marketing manager';

function hasMarketingManagerJob(jobName: string | null | undefined): boolean {
...
router.get('/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      await database.query('SELECT 1 FROM marketing_manager_game_sessions LIMIT 1');
    } catch (tableError) {
      return res.json({
...
router.post('/start', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (isTest) return res.json({ session: { id: 0 } });
    const userId = req.user!.id;
    const user = await database.get(`
...
router.post('/submit', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (isTest) return res.json({ success: true, earnings: 0, experience_points: 0, new_level: null, isNewHighScore: false });
    ...
    if (account && totalEarnings > 0) {
      ...
      if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
        ...
        await database.query(
          ...
          [schoolId, userClass, totalEarnings, 'withdrawal', `Marketing Manager Campaign Payout to ${req.user!.username}`, userId]
        );
      }
      await database.query(`
