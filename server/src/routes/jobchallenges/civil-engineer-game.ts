...
function hasCivilEngineerJob(jobName: string | null | undefined): boolean {
...
router.get('/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      await database.query('SELECT 1 FROM civil_engineer_game_sessions LIMIT 1');
    } catch (tableError) {
      return res.json({
...
router.post('/submit', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (Date.now() - sessionPlayedAt < 45000) {
      return res.status(400).json({ error: 'Game submitted too quickly. Each infrastructure project must run for at least 60 seconds.' });
    }
    ...
    if (account && totalEarnings > 0) {
      ...
      if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
        ...
        await database.query(
          ...
          [schoolId, userClass, totalEarnings, 'withdrawal', `Civil Engineer Infrastructure Project Payout to ${req.user!.username}`, userId]
        );
      }
      await database.query(`
