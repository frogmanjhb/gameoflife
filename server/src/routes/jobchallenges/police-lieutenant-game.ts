...
const POLICE_LIEUTENANT_JOB_NAME = 'assistant police lieutenant';

function hasPoliceLieutenantJob(jobName: string | null | undefined): boolean {
  return (jobName || '').toLowerCase().trim() === POLICE_LIEUTENANT_JOB_NAME;
}
...
router.get('/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      await database.query('SELECT 1 FROM police_lieutenant_game_sessions LIMIT 1');
    } catch (tableError) {
      return res.json({
...
router.post('/submit', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (Date.now() - sessionPlayedAt < 45000) {
      return res.status(400).json({ error: 'Game submitted too quickly. Each compliance review must run for at least 60 seconds.' });
    }
    ...
    if (account && totalEarnings > 0) {
      ...
      if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
        ...
        await database.query(
          ...
          [schoolId, userClass, totalEarnings, 'withdrawal', `Police Lieutenant Enforcement Payout to ${req.user!.username}`, userId]
        );
      }
      await database.query(`
