...
function hasTownPlannerJob(jobName: string | null | undefined): boolean {
...
router.get('/status', authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    try {
      await database.query('SELECT 1 FROM town_planner_game_sessions LIMIT 1');
    } catch (tableError) {
      return res.json({
    ...
    if (account && totalEarnings > 0) {
      ...
      if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
        ...
        await database.query(
          ...
          [schoolId, userClass, totalEarnings, 'withdrawal', `Town Planner Planning Proposal Payout to ${req.user!.username}`, userId]
        );
      }
      await database.query(`
