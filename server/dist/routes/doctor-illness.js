...
async function countClassAssignmentsToday(schoolId, townClass) {
    const row = await database_prod_1.default.get(`SELECT COUNT(*)::int AS count FROM doctor_illness_assignments
     ...
     AND assigned_at >= (${doctor_illness_1.DOCTOR_ILLNESS_DAY_START_SQL})`, [townClass, schoolId]);
    return row?.count ?? 0;
}
       ...
       LEFT JOIN jobs j ON u.job_id = j.id WHERE u.id = $1`, [doctorId]);
        ...
        if (used >= doctor_illness_1.DOCTOR_ILLNESS_DAILY_LIMIT) {
            return res.status(400).json({
                error: `Your town class has already reached the daily limit of ${doctor_illness_1.DOCTOR_ILLNESS_DAILY_LIMIT} sick students.`,
            });
        }
...
router.get('/my-status', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const row = await database_prod_1.default.get(`SELECT illness_type, assigned_at, cure_requested_at, cure_fee,
...
router.post('/see-doctor', auth_1.authenticateToken, async (req, res) => {
    try {
       ...
       ORDER BY a.assigned_at DESC LIMIT 1`, [req.user.id]);
        ...
        const paidByInsurance = await (0, insurance_1.hasActiveApprovedHealthInsurance)(req.user.id);
        if (paidByInsurance) {
       ...
       SET cure_requested_at = CURRENT_TIMESTAMP,
           ...
           paid_by_insurance = $2
       WHERE id = $1`, [row.id, paidByInsurance]);
        res.json({
            success: true,
            cured: false,
            pending_cure: true,
