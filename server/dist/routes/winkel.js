...
router.get('/can-purchase', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const existingPurchase = await database_prod_1.default.get('SELECT id FROM shop_purchases WHERE user_id = $1 AND week_start_date = $2', [req.user.id, weekStart]);
...
router.post('/purchase', auth_1.authenticateToken, [
    ...
    try {
        ...
        // Start transaction
        await database_prod_1.default.run('BEGIN');
        try {
            // Deduct from student account
            await database_prod_1.default.run('UPDATE accounts SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [price, account.id]);
            // Add to shop account
            await database_prod_1.default.run('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [price, shopAccount.id]);
            // Update shop balance table (always id=1)
            await database_prod_1.default.run(`UPDATE shop_balance SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = 1`, [price]);
            // Record transaction (from student to shop)
            await database_prod_1.default.run(`INSERT INTO transactions (from_account_id, to_account_id, amount, transaction_type, description) 
           VALUES ($1, $2, $3, $4, $5)`, [account.id, shopAccount.id, price, 'transfer', `Shop Purchase: ${item.name}`]);
            // Record purchase
            const purchaseDate = formatDate(new Date());
            try {
                await database_prod_1.default.run(`INSERT INTO shop_purchases (user_id, item_id, price_paid, purchase_date, week_start_date) 
