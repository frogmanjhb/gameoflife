...
async function getWeeklyPurchaseLimit() {
    try {
        const setting = await database_prod_1.default.get('SELECT setting_value FROM bank_settings WHERE setting_key = $1', ['weekly_purchase_limit']);
...
router.get('/items', auth_1.authenticateToken, async (req, res) => {
    try {
        const items = await database_prod_1.default.query('SELECT * FROM shop_items WHERE available = true ORDER BY category, price');
        res.json(items);
    }
    catch (error) {
...
router.get('/items/all', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const items = await database_prod_1.default.query('SELECT * FROM shop_items ORDER BY category, name');
        res.json(items);
    }
    catch (error) {
...
router.post('/items', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), [
    (0, express_validator_1.body)('name').trim().notEmpty().withMessage('Name is required'),
    (0, express_validator_1.body)('category').isIn(['consumable', 'privilege', 'profile']).withMessage('Invalid category'),
    ...
    (0, express_validator_1.body)('event_day_only').optional().isBoolean(),
], async (req, res) => {
    try {
        ...
        const result = await database_prod_1.default.query(`INSERT INTO shop_items (name, category, price, description, notes, available, event_day_only)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         RETURNING *`, [name, category, price, description || null, notes || null, available, event_day_only]);
...
router.put('/items/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), [
    (0, express_validator_1.body)('name').optional().trim().notEmpty().withMessage('Name cannot be empty'),
    (0, express_validator_1.body)('category').optional().isIn(['consumable', 'privilege', 'profile']).withMessage('Invalid category'),
    ...
    try {
        ...
        const existingItem = await database_prod_1.default.get('SELECT * FROM shop_items WHERE id = $1', [id]);
        if (!existingItem) {
        ...
        values.push(id);
        const result = await database_prod_1.default.query(`UPDATE shop_items SET ${updates.join(', ')} WHERE id = $${paramIndex} RETURNING *`, values);
        res.json(result[0]);
    }
    catch (error) {
...
router.delete('/items/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const { id } = req.params;
        // Check if item exists
        const existingItem = await database_prod_1.default.get('SELECT * FROM shop_items WHERE id = $1', [id]);
        ...
        const purchaseCount = await database_prod_1.default.get('SELECT COUNT(*) as count FROM shop_purchases WHERE item_id = $1', [id]);
        if (parseInt(purchaseCount.count) > 0) {
            // Instead of deleting, just mark as unavailable to preserve purchase history
            await database_prod_1.default.run('UPDATE shop_items SET available = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1', [id]);
            return res.json({
        ...
        await database_prod_1.default.run('DELETE FROM shop_items WHERE id = $1', [id]);
...
router.get('/purchases', auth_1.authenticateToken, tenant_1.requireTenant, async (req, res) => {
    try {
        if (!req.user) {
        ...
        if (req.user.role === 'student') {
            ...
            purchases = await database_prod_1.default.query(`SELECT 
          sp.*,
         ...
         WHERE sp.user_id = $1
         ORDER BY sp.purchase_date DESC, sp.created_at DESC`, [req.user.id]);
        }
        else {
           ...
           ORDER BY COALESCE(sp.paid, false) ASC, sp.purchase_date DESC, sp.created_at DESC`, purchaseParams);
            }
            catch (queryError) {
           ...
           JOIN users u ON sp.user_id = u.id
           WHERE 1=1 ${schoolFilter}
           ORDER BY sp.purchase_date DESC, sp.created_at DESC`, purchaseParams);
            }
        }
        res.json(purchases);
    }
...
router.put('/settings', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), [
    (0, express_validator_1.body)('weekly_purchase_limit').optional().isInt({ min: 1, max: 10 }).withMessage('Weekly limit must be between 1 and 10'),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        if (weekly_purchase_limit !== undefined) {
            ...
            const existing = await database_prod_1.default.get('SELECT * FROM bank_settings WHERE setting_key = $1', ['weekly_purchase_limit']);
            if (existing) {
                await database_prod_1.default.run('UPDATE bank_settings SET setting_value = $1, updated_at = CURRENT_TIMESTAMP, updated_by = $2 WHERE setting_key = $3', [weekly_purchase_limit.toString(), req.user?.id, 'weekly_purchase_limit']);
            }
            else {
                await database_prod_1.default.run('INSERT INTO bank_settings (setting_key, setting_value, description, updated_by) VALUES ($1, $2, $3, $4)', ['weekly_purchase_limit', weekly_purchase_limit.toString(), 'Maximum purchases per student per week', req.user?.id]);
            }
        }
...
router.get('/can-purchase', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const purchaseCount = await database_prod_1.default.get(`SELECT COUNT(*) as count FROM shop_purchases sp
       JOIN shop_items si ON sp.item_id = si.id
       WHERE sp.user_id = $1 AND sp.week_start_date = $2 AND si.category != 'profile'`, [req.user.id, weekStart]);
...
router.post('/purchase', auth_1.authenticateToken, [
    (0, express_validator_1.body)('item_id').isInt().withMessage('Item ID must be an integer'),
], async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'User not found' });
        ...
        // Get the item
        const item = await database_prod_1.default.get('SELECT * FROM shop_items WHERE id = $1 AND available = true', [item_id]);
        if (!item) {
        ...
        if (item.category === 'profile') {
            ...
            const existingEmojiPurchase = await database_prod_1.default.get('SELECT id FROM shop_purchases WHERE user_id = $1 AND item_id = $2', [req.user.id, item_id]);
        ...
        const activeLoan = await database_prod_1.default.get(`SELECT id, monthly_payment, due_date, outstanding_balance 
         FROM loans 
         WHERE borrower_id = $1 AND status = 'active' AND due_date IS NOT NULL AND due_date < CURRENT_DATE`, [req.user.id]);
        if (activeLoan) {
            return res.status(400).json({
        ...
        let shopAccount = await database_prod_1.default.get(`SELECT a.* FROM accounts a 
         JOIN users u ON a.user_id = u.id 
         WHERE u.username = 'shop_system' LIMIT 1`);
        if (!shopAccount) {
            // Create shop system user and account if it doesn't exist
            const shopUser = await database_prod_1.default.get('SELECT id FROM users WHERE username = $1', ['shop_system']);
            let shopUserId;
            if (!shopUser) {
                const newShopUserResult = await database_prod_1.default.query(`INSERT INTO users (username, password_hash, role, first_name, last_name) 
             VALUES ($1, $2, $3, $4, $5) RETURNING id`, ['shop_system', 'system_account', 'teacher', 'Shop', 'System']);
            ...
            if (shopUserId) {
                ...
                await database_prod_1.default.run(`INSERT INTO accounts (user_id, account_number, balance) 
             VALUES ($1, $2, $3)`, [shopUserId, accountNumber, 0.00]);
                // Get the newly created account
                const accounts = await database_prod_1.default.query(`SELECT a.* FROM accounts a WHERE a.user_id = $1`, [shopUserId]);
        ...
        try {
            ...
            const accountResult = await client.query('SELECT * FROM accounts WHERE user_id = $1 FOR UPDATE', [req.user.id]);
            ...
            await client.query('UPDATE accounts SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [price, account.id]);
            // Add to shop account (lock it too)
            await client.query('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [price, shopAccount.id]);
            // Update shop balance table (always id=1)
            await client.query(`UPDATE shop_balance SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = 1`, [price]);
            ...
            if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
                if (shopSchoolId != null) {
                    await client.query('UPDATE town_settings SET treasury_balance = treasury_balance + $1, updated_at = CURRENT_TIMESTAMP WHERE class = $2 AND school_id = $3', [price, userClass, shopSchoolId]);
                }
                else {
                    await client.query('UPDATE town_settings SET treasury_balance = treasury_balance + $1, updated_at = CURRENT_TIMESTAMP WHERE class = $2 AND school_id IS NULL', [price, userClass]);
                }
                // Record treasury transaction
                await client.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [shopSchoolId, userClass, price, 'deposit', `Shop Purchase: ${item.name} by ${req.user.username}`, req.user.id]);
            }
           ...
           VALUES ($1, $2, $3, $4, $5)`, [account.id, shopAccount.id, price, 'transfer', `Shop Purchase: ${item.name}`]);
            ...
            try {
                await client.query(`INSERT INTO shop_purchases (user_id, item_id, price_paid, purchase_date, week_start_date) 
             VALUES ($1, $2, $3, $4, $5)`, [req.user.id, item_id, price, purchaseDate, weekStart]);
            }
            catch (insertError) {
            ...
            if (item.category === 'profile' && item.name) {
                ...
                if (emojiMatch) {
                    ...
                    await client.query('UPDATE users SET profile_emoji = $1 WHERE id = $2', [emoji, req.user.id]);
                }
            }
            await client.query('COMMIT');
            // Get updated account balance (outside transaction is fine)
            const updatedAccount = await database_prod_1.default.get('SELECT balance FROM accounts WHERE id = $1', [account.id]);
            res.json({
...
router.get('/balance', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const shopBalance = await database_prod_1.default.get('SELECT balance FROM shop_balance WHERE id = 1');
        res.json({
...
router.get('/stats', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const schoolId = req.schoolId ?? req.user?.school_id ?? null;
        ...
        const stats = await database_prod_1.default.query(`SELECT 
        si.category,
        si.name,
       ...
       ORDER BY si.category, purchase_count DESC`, params);
        const totalPurchasesQ = schoolId !== null
            ? database_prod_1.default.get('SELECT COUNT(*) as count FROM shop_purchases sp JOIN users u ON sp.user_id = u.id WHERE u.school_id = $1', [schoolId])
            : database_prod_1.default.get('SELECT COUNT(*) as count FROM shop_purchases');
        const totalPurchases = await totalPurchasesQ;
        const totalRevenueQ = schoolId !== null
            ? database_prod_1.default.get('SELECT COALESCE(SUM(sp.price_paid), 0) as total FROM shop_purchases sp JOIN users u ON sp.user_id = u.id WHERE u.school_id = $1', [schoolId])
            : database_prod_1.default.get('SELECT COALESCE(SUM(price_paid), 0) as total FROM shop_purchases');
...
router.post('/change-emoji', auth_1.authenticateToken, [
    (0, express_validator_1.body)('item_id').isInt().withMessage('Item ID must be an integer'),
], async (req, res) => {
    try {
        if (!req.user) {
        ...
        const { item_id } = req.body;
        // Get the item
        const item = await database_prod_1.default.get('SELECT * FROM shop_items WHERE id = $1 AND category = $2', [item_id, 'profile']);
        if (!item) {
            return res.status(404).json({ error: 'Profile emoji not found' });
        }
        // Check if student owns this emoji
        const purchase = await database_prod_1.default.get('SELECT id FROM shop_purchases WHERE user_id = $1 AND item_id = $2', [req.user.id, item_id]);
        if (!purchase) {
        ...
        const emoji = emojiMatch[1];
        // Update user profile
        await database_prod_1.default.run('UPDATE users SET profile_emoji = $1 WHERE id = $2', [emoji, req.user.id]);
        res.json({
...
router.put('/purchases/:id/paid', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const { id } = req.params;
        const schoolId = req.schoolId ?? req.user?.school_id ?? null;
        // Check if purchase exists and purchaser is in teacher's school
        const purchase = await database_prod_1.default.get(`SELECT sp.* FROM shop_purchases sp
         JOIN users u ON sp.user_id = u.id
         WHERE sp.id = $1 ${schoolId !== null ? 'AND u.school_id = $2' : ''}`, schoolId !== null ? [id, schoolId] : [id]);
        if (!purchase) {
        ...
        // Mark as paid
        await database_prod_1.default.run('UPDATE shop_purchases SET paid = true, paid_at = CURRENT_TIMESTAMP, paid_by = $1 WHERE id = $2', [req.user?.id, id]);
        // Get updated purchase with details
        const updatedPurchase = await database_prod_1.default.get(`SELECT 
          sp.*,
          si.name as item_name,
          si.category as item_category,
          u.username,
...
router.get('/owned-emojis', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can view owned emojis' });
        }
        const ownedEmojis = await database_prod_1.default.query(`SELECT 
        si.id,
        si.name,
        si.description,
        sp.purchase_date
       ...
       ORDER BY sp.purchase_date DESC`, [req.user.id]);
...
//# sourceMappingURL=winkel.js.map