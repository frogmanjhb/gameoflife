...
router.post('/:id/pay', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        const account = await database_prod_1.default.get('SELECT * FROM accounts WHERE user_id = $1', [tender.awarded_to_user_id]);
        if (!account) {
        ...
        try {
            ...
            // Record transaction
            await client.query('INSERT INTO transactions (to_account_id, amount, transaction_type, description) VALUES ($1, $2, $3, $4)', [account.id, amount, 'deposit', `Tender payout: ${tender.name}`]);
            // Mark tender as paid
            await client.query('UPDATE tenders SET paid = true, paid_at = CURRENT_TIMESTAMP, paid_by = $1 WHERE id = $2', [req.user.id, tenderId]);
        ...
        const updatedTender = await database_prod_1.default.get(`SELECT 
          t.*,
...
router.post('/', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), [
    ...
    (0, express_validator_1.body)('value').isFloat({ min: 0 }).withMessage('Value must be 0 or greater')
], async (req, res) => {
    try {
...
router.post('/:id/apply', auth_1.authenticateToken, (0, auth_1.requireRole)(['student']), async (req, res) => {
    try {
        const tenderId = parseInt(req.params.id);
        if (isNaN(tenderId)) {
            return res.status(400).json({ error: 'Invalid tender ID' });
        ...
        const result = await database_prod_1.default.run(`INSERT INTO tender_applications (tender_id, applicant_id, status)
       VALUES ($1, $2, 'pending')
       ON CONFLICT (tender_id, applicant_id) DO NOTHING
       RETURNING id`, [tenderId, req.user.id]);
        // If it already existed, fetch existing app
        const applicationId = result.lastID;
        const application = applicationId
            ? await database_prod_1.default.get(`SELECT ta.*, u.username AS applicant_username, u.first_name AS applicant_first_name, u.last_name AS applicant_last_name
           ...
           FROM tender_applications ta
           JOIN users u ON ta.applicant_id = u.id
           WHERE ta.tender_id = $1 AND ta.applicant_id = $2`, [tenderId, req.user.id]);
...
router.get('/:id/applications', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
       ...
       ORDER BY ta.created_at DESC`, [tenderId]);
...
router.put('/applications/:id', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), [(0, express_validator_1.body)('status').isIn(['approved', 'denied']).withMessage('Status must be approved or denied')], async (req, res) => {
    ...
    try {
        ...
        if (status === 'approved') {
            ...
            const alreadyApproved = await database_prod_1.default.get(`SELECT id FROM tender_applications WHERE tender_id = $1 AND status = 'approved' LIMIT 1`, [tender.id]);
            if (alreadyApproved) {
        ...
        try {
            ...
            await client.query(`UPDATE tender_applications
           SET status = $1, reviewed_by = $2, reviewed_at = CURRENT_TIMESTAMP
           WHERE id = $3`, [status, req.user.id, applicationId]);
            // If approved, award tender and deny other pending applications
            if (status === 'approved') {
                await client.query(`UPDATE tenders
             SET status = 'awarded',
                 awarded_to_user_id = $1,
                 awarded_application_id = $2,
                 awarded_at = CURRENT_TIMESTAMP
             WHERE id = $3`, [application.applicant_id, applicationId, tender.id]);
                await client.query(`UPDATE tender_applications
             SET status = 'denied',
                 reviewed_by = $1,
                 reviewed_at = CURRENT_TIMESTAMP
             WHERE tender_id = $2 AND id <> $3 AND status = 'pending'`, [req.user.id, tender.id, applicationId]);
            }
            await client.query('COMMIT');
        }
        catch (txErr) {
        ...
        const updated = await database_prod_1.default.get(`SELECT ta.*, r.username AS reviewer_username
         FROM tender_applications ta
         LEFT JOIN users r ON ta.reviewed_by = r.id
         WHERE ta.id = $1`, [applicationId]);
