...
async function syncParcelAppreciation(parcelId) {
    ...
    const currentValue = (0, landProperty_1.calculateAppreciatedValue)(purchasePrice, parcel.purchased_at);
    if (currentValue !== Number(parcel.value)) {
        await database_prod_1.default.run('UPDATE land_parcels SET value = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [currentValue, parcelId]);
    }
}
...
async function assertFinancialManager(userId, schoolId, townClass) {
    const user = await getUserWithJob(userId);
    if (!user || !(0, landProperty_1.hasFinancialManagerJob)(user.job_name)) {
        return { ok: false, error: 'Only the Financial Manager can perform this action' };
    }
    if ((user.school_id ?? null) !== schoolId) {
        return { ok: false, error: 'Financial Manager must be in the same school' };
    }
    if (user.class !== townClass) {
        return { ok: false, error: 'Financial Manager must be in the same town class as the property' };
    }
    return { ok: true, user };
}
...
router.post('/collect-rent/:parcelId', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const refreshed = await database_prod_1.default.get('SELECT * FROM land_parcels WHERE id = $1', [parcelId]);
        const purchasePrice = Number(refreshed.purchase_price ?? refreshed.value);
        const currentValue = (0, landProperty_1.calculateAppreciatedValue)(purchasePrice, refreshed.purchased_at);
        if (!(0, landProperty_1.canCollectWeeklyRent)(refreshed.purchased_at, refreshed.last_rent_collected_at)) {
            return res.status(400).json({ error: 'Rental income already collected for this week' });
        }
        const rentAmount = (0, landProperty_1.calculateWeeklyRent)(currentValue);
        const account = await database_prod_1.default.get('SELECT id FROM accounts WHERE user_id = $1', [req.user.id]);
        if (!account) {
            return res.status(404).json({ error: 'Account not found' });
        }
        await database_prod_1.default.run('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE user_id = $2', [rentAmount, req.user.id]);
        await database_prod_1.default.run(`INSERT INTO transactions (to_account_id, amount, transaction_type, description)
       VALUES ($1, $2, 'deposit', $3)`, [account.id, rentAmount, `Land rental income: Plot ${refreshed.grid_code}`]);
        await database_prod_1.default.run('UPDATE land_parcels SET last_rent_collected_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = $1', [parcelId]);
        res.json({
            message: 'Rental income collected',
            amount: rentAmount,
            parcel: (0, landProperty_1.enrichOwnedParcel)({ ...refreshed, value: currentValue }),
        });
    }
    catch (error) {
...
router.post('/sale-requests', auth_1.authenticateToken, (0, express_validator_1.body)('parcel_id').isInt({ min: 1 }), (0, express_validator_1.body)('buyer_id').isInt({ min: 1 }), (0, express_validator_1.body)('sale_price').isFloat({ min: 0.01 }), async (req, res) => {
    try {
        ...
        if ((0, landProperty_1.isTownClass)(userClass) && parcel.town_class !== userClass) {
            return res.status(403).json({ error: 'You can only sell land in your town' });
        }
        const buyer = await database_prod_1.default.get(`SELECT id, class, school_id FROM users WHERE id = $1 AND role = 'student'`, [buyer_id]);
        if (!buyer) {
        ...
        const activeSale = await database_prod_1.default.get(`SELECT id FROM land_sale_requests WHERE parcel_id = $1 AND status IN ('pending_fm', 'pending_buyer')`, [parcel_id]);
        if (activeSale) {
        ...
        const result = schoolId !== null
            ? await database_prod_1.default.get(`INSERT INTO land_sale_requests (parcel_id, seller_id, buyer_id, sale_price, school_id)
             VALUES ($1, $2, $3, $4, $5) RETURNING id`, [parcel_id, req.user.id, buyer_id, sale_price, schoolId])
            : await database_prod_1.default.get(`INSERT INTO land_sale_requests (parcel_id, seller_id, buyer_id, sale_price)
             VALUES ($1, $2, $3, $4) RETURNING id`, [parcel_id, req.user.id, buyer_id, sale_price]);
        const request = await database_prod_1.default.get(`${saleRequestSelect()} WHERE lsr.id = $1`, [result.id]);
...
router.get('/sale-requests', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (role === 'fm') {
            ...
            if (!fmCheck || !(0, landProperty_1.hasFinancialManagerJob)(fmCheck.job_name)) {
                return res.status(403).json({ error: 'Only Financial Managers can view approval queue' });
            }
            requests = schoolId !== null
                ? await database_prod_1.default.query(`${saleRequestSelect()}
             WHERE lsr.status = 'pending_fm' AND lsr.school_id = $1 AND lp.town_class = $2
             ORDER BY lsr.created_at ASC`, [schoolId, fmCheck.class])
                : await database_prod_1.default.query(`${saleRequestSelect()}
             WHERE lsr.status = 'pending_fm' AND lsr.school_id IS NULL AND lp.town_class = $1
             ORDER BY lsr.created_at ASC`, [fmCheck.class]);
        }
        else if (role === 'buyer') {
            requests = await database_prod_1.default.query(`${saleRequestSelect()} WHERE lsr.buyer_id = $1 ORDER BY lsr.created_at DESC`, [req.user.id]);
        }
        else {
            requests = await database_prod_1.default.query(`${saleRequestSelect()} WHERE lsr.seller_id = $1 ORDER BY lsr.created_at DESC`, [req.user.id]);
        }
        res.json(requests);
    }
    catch (error) {
...
router.put('/sale-requests/:id/fm-review', auth_1.authenticateToken, (0, express_validator_1.body)('status').isIn(['pending_buyer', 'denied']), (0, express_validator_1.body)('denial_reason').optional().isString(), async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        ...
        const sale = await database_prod_1.default.get(`${saleRequestSelect()} WHERE lsr.id = $1`, [requestId]);
        ...
        if (status === 'denied') {
           ...
           WHERE id = $3`, [req.user.id, denial_reason || 'Denied by Financial Manager', requestId]);
        }
        else {
            await database_prod_1.default.run(`UPDATE land_sale_requests
           SET status = 'pending_buyer', fm_reviewed_by = $1, fm_reviewed_at = CURRENT_TIMESTAMP,
               updated_at = CURRENT_TIMESTAMP
           WHERE id = $2`, [req.user.id, requestId]);
        }
        const updated = await database_prod_1.default.get(`${saleRequestSelect()} WHERE lsr.id = $1`, [requestId]);
...
router.put('/sale-requests/:id/accept', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const parcel = await database_prod_1.default.get('SELECT * FROM land_parcels WHERE id = $1', [sale.parcel_id]);
        ...
        const buyerAccountRes = await client.query('SELECT * FROM accounts WHERE user_id = $1 FOR UPDATE', [sale.buyer_id]);
        const sellerAccountRes = await client.query('SELECT * FROM accounts WHERE user_id = $1 FOR UPDATE', [sale.seller_id]);
        ...
        await client.query('UPDATE accounts SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [salePrice, buyerAccount.id]);
        await client.query('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [salePrice, sellerAccount.id]);
        await client.query(`INSERT INTO transactions (from_account_id, to_account_id, amount, transaction_type, description)
       VALUES ($1, $2, $3, 'transfer', $4)`, [buyerAccount.id, sellerAccount.id, salePrice, `Land sale: Plot ${sale.parcel_grid_code}`]);
        await client.query(`UPDATE land_parcels
       SET owner_id = $1, value = $2, purchase_price = $2,
           purchased_at = CURRENT_TIMESTAMP, last_rent_collected_at = NULL,
           updated_at = CURRENT_TIMESTAMP
       WHERE id = $3`, [sale.buyer_id, salePrice, sale.parcel_id]);
        await client.query(`UPDATE land_sale_requests
...
router.put('/sale-requests/:id/cancel', auth_1.authenticateToken, async (req, res) => {
    try {
        const requestId = parseInt(req.params.id, 10);
        const sale = await database_prod_1.default.get('SELECT * FROM land_sale_requests WHERE id = $1', [requestId]);
        ...
        await database_prod_1.default.run(`UPDATE land_sale_requests SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [requestId]);
...
//# sourceMappingURL=land-economy.js.map