    ...
    try {
      ...
      `, [dateInterval, startDateIso, req.schoolId, intervalExpr]);
        ...
        if (loginEventsExists) {
            ...
            const lowLoginRows = await database_prod_1.default.query(`
        ...
        `, [startDateIso, req.schoolId]);
            lowLoginStudents = lowLoginRows.map((r) => ({
                id: r.id,
                username: r.username,
      ...
      `, [startDateIso, req.schoolId]);
        ...
        if (scope === 'students') {
            ...
            `
              ...
              LIMIT 50
            `;
                topStudents = await database_prod_1.default.query(studentQuery, className ? [startDateIso, req.schoolId, className] : [startDateIso, req.schoolId]);
            }
            catch (err) {
 ...
 */
router.get('/student-logins', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        if (!req.user || !req.schoolId) {
            return res.status(403).json({ error: 'School context required' });
        }
 ...
 */
router.get('/today-activity', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        if (!req.user || !req.schoolId) {
            return res.status(403).json({ error: 'School context required' });
        }
        ...
        const studentsLoggedInSubquery = loginEventsExists
            ...
            WHERE le.school_id = $1
              ...
              AND u.class IN ('6A', '6B', '6C'))`
            : `(SELECT 0::int)`;
        const row = await database_prod_1.default.get(`
        SELECT
