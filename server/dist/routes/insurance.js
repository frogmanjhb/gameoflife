...
router.get('/broker/pending-claims', auth_1.authenticateToken, tenant_1.requireTenant, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can access broker tools' });
        }
        ...
        const rows = await database_prod_1.default.query(`SELECT a.id, a.illness_type, a.cure_fee, a.insurance_claim_requested_at,
       ...
       ORDER BY a.insurance_claim_requested_at ASC`, [reviewer.class, reviewer.school_id ?? null, req.user.id]);
        res.json(rows);
    }
    catch (error) {
...
router.put('/broker/claims/:assignmentId/review', auth_1.authenticateToken, tenant_1.requireTenant, (0, express_validator_1.body)('status').isIn(['approved', 'denied']), (0, express_validator_1.body)('denial_reason').optional().isString(), async (req, res) => {
    const client = await database_prod_1.default.pool.connect();
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const { status, denial_reason: denialReason } = req.body;
        const assignment = await database_prod_1.default.get(`SELECT a.*, p.username AS patient_username,
                COALESCE(NULLIF(TRIM(CONCAT(p.first_name, ' ', p.last_name)), ''), p.username) AS patient_display_name
         FROM doctor_illness_assignments a
         JOIN users p ON p.id = a.patient_user_id
        ...
        const doctorAccount = await database_prod_1.default.get('SELECT id FROM accounts WHERE user_id = $1', [
...
router.get('/broker/pending-cyber-claims', auth_1.authenticateToken, tenant_1.requireTenant, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can access broker tools' });
        }
        ...
        const rows = await database_prod_1.default.query(`SELECT a.id, a.attack_type, a.repair_fee, a.insurance_claim_requested_at,
...
router.put('/broker/cyber-claims/:assignmentId/review', auth_1.authenticateToken, tenant_1.requireTenant, (0, express_validator_1.body)('status').isIn(['approved', 'denied']), (0, express_validator_1.body)('denial_reason').optional().isString(), async (req, res) => {
    const client = await database_prod_1.default.pool.connect();
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
        ...
        const { status, denial_reason: denialReason } = req.body;
        const assignment = await database_prod_1.default.get(`SELECT a.*, v.username AS victim_username,
                COALESCE(NULLIF(TRIM(CONCAT(v.first_name, ' ', v.last_name)), ''), v.username) AS victim_display_name
         FROM cyber_attack_assignments a
         JOIN users v ON v.id = a.victim_user_id
         WHERE a.id = $1`, [assignmentId]);
        ...
        await client.query('BEGIN');
        if (status === 'denied') {
            await client.query(`UPDATE cyber_attack_assignments
           SET insurance_claim_requested_at = NULL,
               insurance_claim_reviewed_by = $1,
               insurance_claim_reviewed_at = CURRENT_TIMESTAMP
           WHERE id = $2`, [req.user.id, assignmentId]);
        ...
        const engineerAccount = await database_prod_1.default.get('SELECT id FROM accounts WHERE user_id = $1', [
            assignment.assigned_by_user_id,
        ]);
        if (!engineerAccount) {
            await client.query('ROLLBACK');
        ...
        await (0, insurance_1.payCyberInsuranceRepairClaim)(client, assignmentId, engineerAccount.id, repairFee, assignment.attack_type);
        await client.query(`UPDATE cyber_attack_assignments
         SET insurance_claim_reviewed_by = $1,
             insurance_claim_reviewed_at = CURRENT_TIMESTAMP
         WHERE id = $2`, [req.user.id, assignmentId]);
...
router.get('/type-settings', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const schoolId = req.schoolId ?? req.user?.school_id ?? null;
        if (schoolId === null) {
            return res.status(403).json({ error: 'School context required' });
        }
        const types = await (0, insurance_1.getInsuranceTypeSettings)(schoolId);
        res.json({ types });
    }
    catch (error) {
...
router.put('/type-settings/:type', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), (0, express_validator_1.body)('enabled').isBoolean().withMessage('enabled must be true or false'), async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
        ...
        if (!VALID_TYPES.includes(insuranceType)) {
            return res.status(400).json({ error: 'Invalid insurance type' });
        }
        const { enabled } = req.body;
        if (!enabled) {
            const currentlyEnabled = await (0, insurance_1.getEnabledInsuranceTypes)(schoolId);
            if (currentlyEnabled.length <= 1 && currentlyEnabled.includes(insuranceType)) {
                return res.status(400).json({ error: 'At least one insurance type must remain available' });
            }
        }
        await (0, insurance_1.setInsuranceTypeEnabled)(schoolId, insuranceType, enabled, req.user.id);
        const types = await (0, insurance_1.getInsuranceTypeSettings)(schoolId);
        res.json({ types });
    }
    catch (error) {
...
router.post('/purchases/:id/refund', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    const client = await database_prod_1.default.pool.connect();
    try {
        const schoolId = req.schoolId ?? req.user?.school_id ?? null;
        if (schoolId === null) {
        ...
        const purchase = await database_prod_1.default.get(`SELECT ip.*, u.username AS student_username
         FROM insurance_purchases ip
         JOIN users u ON u.id = ip.user_id
         WHERE ip.id = $1 AND u.school_id = $2 AND u.role = 'student'`, [purchaseId, schoolId]);
        if (!purchase) {
        ...
        if (!(0, insurance_1.canTeacherRefundInsuranceStatus)(status)) {
        ...
        const refundAmount = (0, insurance_1.calculateTeacherRefundAmount)(totalCost);
        if (refundAmount <= 0) {
        ...
        await client.query(`UPDATE insurance_purchases
         SET status = 'refunded',
             ...
             refunded_by = $2
         WHERE id = $3`, [refundAmount, req.user.id, purchaseId]);
        await client.query('COMMIT');
        res.json({
            success: true,
            refund_amount: refundAmount,
            original_cost: totalCost,
            refund_percent: insurance_1.INSURANCE_TEACHER_REFUND_RATE * 100,
