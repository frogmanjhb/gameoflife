...
async function getWordleChoresEnabled() {
    try {
        const row = await database_prod_1.default.get('SELECT setting_value FROM bank_settings WHERE setting_key = $1', ['wordle_chores_enabled']);
...
async function getWordleDailyLimit() {
    try {
        const row = await database_prod_1.default.get('SELECT setting_value FROM bank_settings WHERE setting_key = $1', ['wordle_game_daily_limit']);
...
router.post('/start', auth_1.authenticateToken, async (req, res) => {
    ...
    `, [userId]);
        ...
        const targetWord = (0, wordle_words_1.getRandomWord)();
        const rows = await database_prod_1.default.query(`
...
router.post('/guess', auth_1.authenticateToken, async (req, res) => {
    ...
    `, [session_id, userId]);
        ...
        const target = session.target_word.toLowerCase();
      ...
      `, [totalEarnings, session_id]);
            ...
            if (account && totalEarnings > 0 && userClass && ['6A', '6B', '6C'].includes(userClass)) {
                ...
                if (treasuryBalance >= totalEarnings) {
                    ...
                    await database_prod_1.default.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [schoolId, userClass, totalEarnings, 'withdrawal', `Wordle Chore Payout to ${req.user.username}`, userId]);
                    await database_prod_1.default.query('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [totalEarnings, account.id]);
                    await database_prod_1.default.query('INSERT INTO transactions (to_account_id, amount, transaction_type, description) VALUES ($1, $2, $3, $4)', [account.id, totalEarnings, 'deposit', 'Wordle Chore Earnings']);
                }
            }
            const user = await database_prod_1.default.get('SELECT job_id, job_level, job_experience_points FROM users WHERE id = $1', [userId]);
            if (user?.job_id) {
                ...
                for (let l = currentLevel; l < 10; l++) {
                    if (newXP >= (0, jobs_1.getXPForLevel)(l + 1))
                        level = l + 1;
                    else
                        break;
                }
                await database_prod_1.default.query('UPDATE users SET job_experience_points = $1, job_level = $2 WHERE id = $3', [newXP, level, userId]);
