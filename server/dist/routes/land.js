    ...
    try {
      ...
      `, [req.user.id, parcel_id]);
        ...
        const requiredEngineers = isTownClass(townClassForEngineers)
            ...
            : [];
        const totalCost = (0, landPurchaseApproval_1.calculateTotalPurchaseCost)(offeredAmount, requiredEngineers.length);
        ...
        const hasFm = isTownClass(townClassForEngineers) &&
            (await (0, land_purchase_approval_1.townHasFinancialManager)(userSchoolId, townClassForEngineers));
        ...
        res.status(201).json({
            message: initialStatus === 'pending_fm'
    ...
    try {
        ...
        if (req.user.role === 'student' && role === 'engineer') {
         ...
         FROM users u LEFT JOIN jobs j ON j.id = u.job_id WHERE u.id = $1`, [req.user.id]);
            ...
            const rows = await database_prod_1.default.query(`SELECT lpr.*,
         ...
         ORDER BY lpr.created_at ASC`, [user.class, reviewerSchoolId, userId]);
            const filtered = [];
            for (const row of rows) {
                const rowSchoolId = (0, landPurchaseApproval_1.resolvePurchaseSchoolId)(row.school_id, row.applicant_school_id);
                const requiredForRequest = await (0, land_purchase_approval_1.getRequiredLandEngineers)(rowSchoolId, user.class, row.user_id);
...
router.put('/purchase-requests/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), [
    ...
    (0, express_validator_1.body)('master_approve').optional().isBoolean(),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
      ...
      `, [requestId]);
        ...
        const canApprove = (isMasterApprove && isPendingPurchaseStatus) ||
        ...
        if (status === 'approved' && !canApprove) {
            return res.status(400).json({
                error: currentStatus === 'pending_fm' && !isMasterApprove
        ...
        if (status === 'approved') {
            try {
                await (0, land_purchase_approval_1.executeTeacherPurchaseApproval)(purchaseRequest, req.user.id);
            }
            catch (err) {
...
async function cancelActiveLandRequestsForParcel(parcelId) {
    await database_prod_1.default.run(`UPDATE land_purchase_requests
     SET status = 'denied',
         ...
         updated_at = CURRENT_TIMESTAMP
     WHERE parcel_id = $1 AND status IN ('pending_fm', 'pending_engineer', 'pending_teacher')`, [parcelId]);
