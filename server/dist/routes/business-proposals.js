...
router.post('/', auth_1.authenticateToken, (0, auth_2.requireRole)(['student']), async (req, res) => {
    try {
        ...
        const result = await database_prod_1.default.run(`INSERT INTO business_proposals (user_id, school_id, status, business_name, payload)
       VALUES ($1, $2, 'pending', $3, $4)
       RETURNING id, user_id, status, business_name, payload, created_at`, [userId, schoolId, business_name.trim(), JSON.stringify(payloadObj)]);
        const row = await database_prod_1.default.get('SELECT * FROM business_proposals WHERE id = $1', [result.lastID]);
...
router.get('/my', auth_1.authenticateToken, (0, auth_2.requireRole)(['student']), async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'User not found' });
        const userId = req.user.id;
        const rows = await database_prod_1.default.query(`SELECT id, user_id, status, business_name, payload, created_at, reviewed_at, denial_reason
       FROM business_proposals WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
        res.json(rows);
    }
    catch (error) {
...
router.get('/', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_2.requireRole)(['teacher']), async (req, res) => {
    try {
        const schoolId = req.schoolId ?? req.user?.school_id ?? null;
        if (schoolId == null)
            return res.status(403).json({ error: 'School context required' });
        const rows = await database_prod_1.default.query(`SELECT bp.id, bp.user_id, bp.status, bp.business_name, bp.payload, bp.created_at, bp.reviewed_at, bp.denial_reason,
       ...
       ORDER BY bp.status ASC, bp.created_at DESC`, [schoolId]);
        res.json(rows);
    }
    catch (error) {
...
router.put('/:id', auth_1.authenticateToken, tenant_1.requireTenant, (0, auth_2.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        if (schoolId == null)
            return res.status(403).json({ error: 'School context required' });
        const proposal = await database_prod_1.default.get('SELECT id, user_id, status FROM business_proposals WHERE id = $1 AND school_id = $2', [id, schoolId]);
        if (!proposal)
...
//# sourceMappingURL=business-proposals.js.map