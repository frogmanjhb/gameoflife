...
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        try {
            await database_prod_1.default.query('SELECT 1 FROM disasters LIMIT 1');
        }
        catch (tableError) {
            return res.json([]);
        }
        const disasters = await database_prod_1.default.query(`
      SELECT 
        d.*,
...
router.get('/events/recent', auth_1.authenticateToken, async (req, res) => {
    try {
        // Check if disaster_events table exists
        try {
            await database_prod_1.default.query('SELECT 1 FROM disaster_events LIMIT 1');
        }
        catch (tableError) {
            return res.json([]);
        }
        const events = await database_prod_1.default.query(`
      SELECT 
        de.*,
...
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        const result = await database_prod_1.default.query(`
...
router.put('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        const existing = await database_prod_1.default.get('SELECT * FROM disasters WHERE id = $1', [disasterId]);
        if (!existing) {
...
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        await database_prod_1.default.run('DELETE FROM disasters WHERE id = $1', [disasterId]);
...
router.post('/:id/trigger', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        const disasterId = parseInt(req.params.id);
        ...
        const disaster = await database_prod_1.default.get('SELECT * FROM disasters WHERE id = $1', [disasterId]);
        if (!disaster) {
            return res.status(404).json({ error: 'Disaster not found' });
        }
    ...
    `;
        ...
        for (const student of students) {
            ...
            if (impactAmount !== 0) {
                ...
                await database_prod_1.default.run('UPDATE accounts SET balance = $1, updated_at = NOW() WHERE id = $2', [newBalance, student.account_id]);
                // Record transaction
...
router.put('/:id/toggle', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        await database_prod_1.default.run('UPDATE disasters SET is_active = $1, updated_at = NOW() WHERE id = $2', [newActive, disasterId]);
        const updatedDisaster = await database_prod_1.default.get('SELECT * FROM disasters WHERE id = $1', [disasterId]);
...
//# sourceMappingURL=disasters.js.map