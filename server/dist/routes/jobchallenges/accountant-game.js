...
router.get('/status', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can access accountant game' });
        }
      ...
      WHERE u.id = $1
    `, [userId]);
        if (!user || !hasAccountantJob(user.job_name)) {
            return res.status(403).json({ error: 'Only Chartered Accountants can access this game' });
        }
        ...
        try {
            await database_prod_1.default.query('SELECT 1 FROM accountant_game_sessions LIMIT 1');
        }
        catch (tableError) {
            return res.json({
      ...
      SELECT COUNT(*) as count FROM accountant_game_sessions 
      WHERE user_id = $1 
      AND played_at >= (
        CASE 
    ...
    `, [userId]);
      ...
      FROM accountant_game_high_scores 
      WHERE user_id = $1
    `, [userId]);
      ...
      SELECT * FROM accountant_game_sessions 
      WHERE user_id = $1 
    ...
    `, [userId]);
        ...
        console.error('Get accountant game status error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Start a new audit case (5 questions)
router.post('/start', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can start accountant games' });
        }
    ...
    `, [userId]);
        if (!user || !hasAccountantJob(user.job_name)) {
            return res.status(403).json({ error: 'Only Chartered Accountants can play this game' });
        }
        // Per-town job game daily limit (teacher-configurable in town settings)
        let dailyLimit = config_1.JOB_CHALLENGES_DAILY_LIMIT;
        const userClass = req.user?.class;
        ...
        try {
            ...
            return res.status(503).json({ error: 'Accountant game feature not available yet. Please try again later.' });
        }
        const todayPlays = await database_prod_1.default.query(`
      SELECT COUNT(*) as count FROM accountant_game_sessions 
      WHERE user_id = $1 
    ...
    `, [userId]);
      ...
      INSERT INTO accountant_game_sessions (user_id, difficulty, score, correct_answers, total_problems, experience_points, earnings)
    ...
    `, [userId, difficulty]);
        ...
        console.error('Start accountant game error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
// Submit audit case results
router.post('/submit', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can submit accountant games' });
        }
    ...
    `, [userId]);
        ...
        // Accountant audit game: XP only, no money payout
        const totalEarnings = 0;
        await database_prod_1.default.query(`
      UPDATE accountant_game_sessions 
    ...
    `, [score, correct_answers, total_problems, totalXP, totalEarnings, session_id]);
      ...
      SELECT high_score FROM accountant_game_high_scores 
      WHERE user_id = $1 AND difficulty = $2
    `, [userId, session.difficulty]);
        if (!currentHighScore || score > currentHighScore.high_score) {
            await database_prod_1.default.query(`
        INSERT INTO accountant_game_high_scores (user_id, difficulty, high_score)
        VALUES ($1, $2, $3)
      ...
      `, [userId, session.difficulty, score]);
        ...
        console.error('Submit accountant game error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
exports.default = router;
//# sourceMappingURL=accountant-game.js.map