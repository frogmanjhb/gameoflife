...
router.get('/status', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can access civil engineer game' });
        }
      ...
      WHERE u.id = $1
    `, [userId]);
        if (!user || !hasCivilEngineerJob(user.job_name)) {
            return res.status(403).json({ error: 'Only Civil Engineers can access this game' });
        }
        ...
        try {
            await database_prod_1.default.query('SELECT 1 FROM civil_engineer_game_sessions LIMIT 1');
        }
        catch (tableError) {
            return res.json({
      ...
      SELECT COUNT(*) as count FROM civil_engineer_game_sessions 
      WHERE user_id = $1 
      AND played_at >= (
    ...
    `, [userId]);
      ...
      SELECT difficulty, high_score FROM civil_engineer_game_high_scores WHERE user_id = $1
    `, [userId]);
      ...
      SELECT * FROM civil_engineer_game_sessions WHERE user_id = $1 ORDER BY played_at DESC LIMIT 5
    `, [userId]);
        res.json({ remaining_plays: remainingPlays, daily_limit: dailyLimit, high_scores: highScoreMap, recent_sessions: recentSessions });
    }
    catch (error) {
        console.error('Get civil engineer game status error:', error);
...
router.post('/start', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (!isTest && (!req.user || req.user.role !== 'student')) {
            return res.status(403).json({ error: 'Only students can start civil engineer games' });
        }
    ...
    `, [userId]);
        if (!user || !hasCivilEngineerJob(user.job_name)) {
            return res.status(403).json({ error: 'Only Civil Engineers can play this game' });
        }
        let dailyLimit = config_1.JOB_CHALLENGES_DAILY_LIMIT;
        const userClass = req.user?.class;
        const schoolId = req.user?.school_id ?? null;
        ...
        try {
            ...
            return res.status(503).json({ error: 'Civil engineer game feature not available yet. Please try again later.' });
        }
        const todayPlays = await database_prod_1.default.query(`
      SELECT COUNT(*) as count FROM civil_engineer_game_sessions 
      WHERE user_id = $1 
    ...
    `, [userId]);
      ...
      INSERT INTO civil_engineer_game_sessions (user_id, difficulty, score, correct_answers, total_problems, experience_points, earnings)
    ...
    `, [userId, difficulty]);
        ...
        console.error('Start civil engineer game error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/submit', auth_1.authenticateToken, async (req, res) => {
    try {
        const isTest = req.body?.test === true && req.user?.role === 'teacher';
        if (!isTest && (!req.user || req.user.role !== 'student')) {
            return res.status(403).json({ error: 'Only students can submit civil engineer games' });
        }
        if (isTest)
      ...
      SELECT * FROM civil_engineer_game_sessions WHERE id = $1 AND user_id = $2
    `, [session_id, userId]);
        if (!session)
        ...
        if (Date.now() - sessionPlayedAt < minGameDurationMs) {
            return res.status(400).json({ error: 'Game submitted too quickly. Each infrastructure project must run for at least 15 seconds.' });
        }
    ...
    `, [userId]);
      ...
      UPDATE civil_engineer_game_sessions 
    ...
    `, [score, correct_answers, total_problems, totalXP, totalEarnings, session_id]);
      ...
      SELECT high_score FROM civil_engineer_game_high_scores WHERE user_id = $1 AND difficulty = $2
    `, [userId, session.difficulty]);
        if (!currentHighScore || score > currentHighScore.high_score) {
            await database_prod_1.default.query(`
        INSERT INTO civil_engineer_game_high_scores (user_id, difficulty, high_score)
        VALUES ($1, $2, $3)
      ...
      `, [userId, session.difficulty, score]);
        ...
        if (account && totalEarnings > 0) {
            ...
            if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
                ...
                await database_prod_1.default.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [schoolId, userClass, totalEarnings, 'withdrawal', `Civil Engineer Infrastructure Project Payout to ${req.user.username}`, userId]);
            }
            await database_prod_1.default.query(`
      ...
      `, [account.id, totalEarnings, 'CIVIL_ENGINEER_INFRASTRUCTURE_PROJECT_EARN']);
        }
        res.json({
            success: true,
        ...
        console.error('Submit civil engineer game error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
exports.default = router;
//# sourceMappingURL=civil-engineer-game.js.map