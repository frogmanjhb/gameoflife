...
const MARKETING_MANAGER_JOB_NAME = 'marketing manager';
function hasMarketingManagerJob(jobName) {
    return (jobName || '').toLowerCase().trim() === MARKETING_MANAGER_JOB_NAME;
}
router.get('/status', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can access marketing manager game' });
        }
      ...
      WHERE u.id = $1
    `, [userId]);
        if (!user || !hasMarketingManagerJob(user.job_name)) {
            return res.status(403).json({ error: 'Only Marketing Managers can access this game' });
        }
        let dailyLimit = config_1.JOB_CHALLENGES_DAILY_LIMIT;
        const userClass = req.user?.class;
        const schoolId = req.user?.school_id ?? null;
        if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
            try {
                const row = schoolId != null
                    ? await database_prod_1.default.get('SELECT job_game_daily_limit FROM town_settings WHERE class = $1 AND school_id = $2', [userClass, schoolId])
                    : await database_prod_1.default.get('SELECT job_game_daily_limit FROM town_settings WHERE class = $1 AND school_id IS NULL', [userClass]);
                if (row != null && row.job_game_daily_limit != null)
                    dailyLimit = parseInt(String(row.job_game_daily_limit), 10) || dailyLimit;
            }
            catch (_) { /* use default */ }
        }
        try {
            await database_prod_1.default.query('SELECT 1 FROM marketing_manager_game_sessions LIMIT 1');
        }
        catch (tableError) {
            return res.json({
      ...
      SELECT COUNT(*) as count FROM marketing_manager_game_sessions 
      WHERE user_id = $1 
      AND played_at >= (
    ...
    `, [userId]);
      ...
      SELECT difficulty, high_score FROM marketing_manager_game_high_scores WHERE user_id = $1
    `, [userId]);
        const highScoreMap = { easy: 0, medium: 0, hard: 0, extreme: 0 };
        highScores.forEach((row) => {
            const d = row.difficulty;
            if (d in highScoreMap)
                highScoreMap[d] = row.high_score;
        });
        const recentSessions = await database_prod_1.default.query(`
      SELECT * FROM marketing_manager_game_sessions WHERE user_id = $1 ORDER BY played_at DESC LIMIT 5
    `, [userId]);
        res.json({ remaining_plays: remainingPlays, daily_limit: dailyLimit, high_scores: highScoreMap, recent_sessions: recentSessions });
    }
    catch (error) {
        console.error('Get marketing manager game status error:', error);
...
router.post('/start', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can start marketing manager games' });
        }
    ...
    `, [userId]);
        if (!user || !hasMarketingManagerJob(user.job_name)) {
            return res.status(403).json({ error: 'Only Marketing Managers can play this game' });
        }
        let dailyLimit = config_1.JOB_CHALLENGES_DAILY_LIMIT;
        const userClass = req.user?.class;
        const schoolId = req.user?.school_id ?? null;
        ...
        try {
            ...
            return res.status(503).json({ error: 'Marketing manager game feature not available yet. Please try again later.' });
        }
        const todayPlays = await database_prod_1.default.query(`
      SELECT COUNT(*) as count FROM marketing_manager_game_sessions 
      WHERE user_id = $1 
    ...
    `, [userId]);
        ...
        const session = await database_prod_1.default.query(`
      INSERT INTO marketing_manager_game_sessions (user_id, difficulty, score, correct_answers, total_problems, experience_points, earnings)
    ...
    `, [userId, difficulty]);
        ...
        console.error('Start marketing manager game error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
router.post('/submit', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user || req.user.role !== 'student') {
            return res.status(403).json({ error: 'Only students can submit marketing manager games' });
        }
        ...
        if (safeAnswerSequence.length !== total_problems ||
            (safeAnswerSequence.length > 0 && !safeAnswerSequence.every((a) => typeof a === 'boolean'))) {
      ...
      SELECT * FROM marketing_manager_game_sessions WHERE id = $1 AND user_id = $2
    `, [session_id, userId]);
        if (!session)
        ...
        if (Date.now() - sessionPlayedAt < 45000) {
            return res.status(400).json({ error: 'Game submitted too quickly. Each campaign must run for at least 60 seconds.' });
        }
    ...
    `, [userId]);
        ...
        const difficultyMultipliers = { easy: 1.0, medium: 1.5, hard: 2.0, extreme: 3.0 };
      ...
      UPDATE marketing_manager_game_sessions 
    ...
    `, [score, correct_answers, total_problems, totalXP, totalEarnings, session_id]);
        const currentHighScore = await database_prod_1.default.get(`
      SELECT high_score FROM marketing_manager_game_high_scores WHERE user_id = $1 AND difficulty = $2
    `, [userId, session.difficulty]);
        if (!currentHighScore || score > currentHighScore.high_score) {
            await database_prod_1.default.query(`
        INSERT INTO marketing_manager_game_high_scores (user_id, difficulty, high_score)
        VALUES ($1, $2, $3)
        ...
        if (account && totalEarnings > 0) {
            ...
            if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
                const townSettings = schoolId != null
                    ? await database_prod_1.default.get('SELECT treasury_balance FROM town_settings WHERE class = $1 AND school_id = $2', [userClass, schoolId])
                ...
                if (schoolId != null) {
                    await database_prod_1.default.query('UPDATE town_settings SET treasury_balance = treasury_balance - $1, updated_at = CURRENT_TIMESTAMP WHERE class = $2 AND school_id = $3', [totalEarnings, userClass, schoolId]);
                }
                else {
                ...
                await database_prod_1.default.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [schoolId, userClass, totalEarnings, 'withdrawal', `Marketing Manager Campaign Payout to ${req.user.username}`, userId]);
            }
            await database_prod_1.default.query(`
      ...
      `, [account.id, totalEarnings, 'MARKETING_MANAGER_CAMPAIGN_EARN']);
        }
        res.json({
            success: true,
        ...
        console.error('Submit marketing manager game error:', error);
        res.status(500).json({ error: 'Internal server error' });
    }
});
exports.default = router;
//# sourceMappingURL=marketing-manager-game.js.map