...
function hasMarketingManagerJob(jobName) {
    return (jobName || '').toLowerCase().trim().includes('marketing manager');
}
...
router.post('/submit', auth_1.authenticateToken, async (req, res) => {
    ...
    `, [session_id, userId]);
        ...
        if (Date.now() - sessionPlayedAt < minGameDurationMs) {
            return res.status(400).json({ error: 'Game submitted too quickly. Each campaign must run for at least 15 seconds.' });
        }
