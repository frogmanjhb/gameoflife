...
router.get('/status', auth_1.authenticateToken, async (req, res) => {
    ...
    `, [userId]);
        ...
        try {
            await database_prod_1.default.query('SELECT 1 FROM software_engineer_game_sessions LIMIT 1');
        }
        catch (tableError) {
            return res.json({
...
router.post('/submit', auth_1.authenticateToken, async (req, res) => {
    ...
    `, [session_id, userId]);
        ...
        if (Date.now() - sessionPlayedAt < minGameDurationMs) {
            return res.status(400).json({ error: 'Game submitted too quickly. Each build sprint must run for at least 5 seconds.' });
        }
      ...
      `, [userId, session.difficulty, score]);
        ...
        if (account && totalEarnings > 0) {
            ...
            if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
                ...
                await database_prod_1.default.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [schoolId, userClass, totalEarnings, 'withdrawal', `Software Engineer Build Payout to ${req.user.username}`, userId]);
            }
            await database_prod_1.default.query(`
...
//# sourceMappingURL=software-engineer-game.js.map