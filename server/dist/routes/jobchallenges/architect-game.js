...
router.get('/status', auth_1.authenticateToken, async (req, res) => {
    try {
      ...
      WHERE u.id = $1
    `, [userId]);
        if (!user || !(user.job_name || '').toLowerCase().trim().includes('architect')) {
        ...
        try {
            await database_prod_1.default.query('SELECT 1 FROM architect_game_sessions LIMIT 1');
        }
        catch (tableError) {
    ...
    `, [userId, session.difficulty]);
      ...
      `, [userId, session.difficulty, score]);
        ...
        for (let level = currentLevel; level < 10; level++) {
            const xpForNextLevel = (0, jobs_1.getXPForLevel)(level + 1);
        ...
        if (account && totalEarnings > 0) {
            ...
            if (userClass && ['6A', '6B', '6C'].includes(userClass)) {
                ...
                const townSettings = architectSchoolId != null
                    ? await database_prod_1.default.get('SELECT treasury_balance FROM town_settings WHERE class = $1 AND school_id = $2', [userClass, architectSchoolId])
                ...
                if (architectSchoolId != null) {
                    await database_prod_1.default.query('UPDATE town_settings SET treasury_balance = treasury_balance - $1, updated_at = CURRENT_TIMESTAMP WHERE class = $2 AND school_id = $3', [totalEarnings, userClass, architectSchoolId]);
                }
                else {
                ...
                await database_prod_1.default.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [architectSchoolId, userClass, totalEarnings, 'withdrawal', `Architect Game Payout to ${req.user.username}`, userId]);
            }
...
//# sourceMappingURL=architect-game.js.map