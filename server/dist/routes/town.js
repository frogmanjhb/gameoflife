...
router.get('/tax-education', auth_1.authenticateToken, (0, auth_1.requireRole)(['student']), async (req, res) => {
    try {
        ...
        const student = await database_prod_1.default.get(`SELECT
        u.job_id,
       ...
       WHERE u.id = $1 AND u.role = 'student'`, [req.user?.id]);
        if (!student) {
        ...
        for (let lvl = 1; lvl <= 10; lvl++) {
            ...
            const grossSalary = Math.round(baseSalary *
                (1 + (lvl - 1) * 0.7222) *
...
router.get('/tax-report/:class', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
       ...
       WHERE town_class = $1 ${dateFilterPlain}`, [townClass]);
