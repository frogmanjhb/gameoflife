...
router.get('/eligibility', auth_1.authenticateToken, (0, auth_1.requireRole)(['student']), async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'User not found' });
        }
...
router.post('/apply', [
    ...
    (0, express_validator_1.body)('term_weeks').isInt({ min: 1, max: 12 }).withMessage('Term must be between 1 and 12 weeks')
    ...
    try {
       ...
       WHERE u.id = $1`, [req.user.id]);
        ...
        const result = await database_prod_1.default.run(`INSERT INTO loans (
        borrower_id,
        amount,
        term_months,
        term_weeks,
...
router.post('/approve', [
    ...
    try {
        ...
        if (approved) {
            ...
            approved_at = $1, 
            due_date = $2
          WHERE id = $3`, [new Date().toISOString(), nextMonday.toISOString().split('T')[0], loan_id]);
                ...
                await client.query('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [loanAmount, account.id]);
                // Record transaction
                await client.query('INSERT INTO transactions (to_account_id, amount, transaction_type, description) VALUES ($1, $2, $3, $4)', [account.id, loanAmount, 'loan_disbursement', `Loan disbursement - R${loanAmount.toFixed(2)}`]);
...
router.post('/process-weekly-payments', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        const loansToProcess = await database_prod_1.default.query(`SELECT l.*, u.username as borrower_username, a.id as account_id, a.balance as account_balance
         FROM loans l
         JOIN users u ON l.borrower_id = u.id
        ...
        for (const loan of loansToProcess) {
            ...
            try {
                ...
                await client.query('UPDATE accounts SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [actualPayment, loan.account_id]);
                // Record loan payment
                await client.query('INSERT INTO loan_payments (loan_id, amount, payment_date) VALUES ($1, $2, CURRENT_TIMESTAMP)', [loan.id, actualPayment]);
                // Record transaction
                await client.query('INSERT INTO transactions (from_account_id, amount, transaction_type, description) VALUES ($1, $2, $3, $4)', [loan.account_id, actualPayment, 'loan_repayment', `Weekly loan payment (automatic) - R${actualPayment.toFixed(2)}`]);
                // Update loan
                if (newOutstandingBalance <= 0.01) {
                    // Loan is paid off
                    await client.query('UPDATE loans SET status = $1, outstanding_balance = 0 WHERE id = $2', ['paid_off', loan.id]);
                    results.push({
                        loan_id: loan.id,
                    ...
                    await client.query('UPDATE loans SET outstanding_balance = $1 WHERE id = $2', [newOutstandingBalance, loan.id]);
                    results.push({
                        loan_id: loan.id,
                        borrower: loan.borrower_username,
                        payment: actualPayment,
...
async function applyStudentLoanPayment(userId, loanId, mode) {
    const loan = await database_prod_1.default.get('SELECT * FROM loans WHERE id = $1 AND borrower_id = $2', [loanId, userId]);
    if (!loan) {
    ...
    if (isNaN(outstandingBalance) || outstandingBalance <= 0.01) {
        if (outstandingBalance <= 0.01) {
            await database_prod_1.default.run('UPDATE loans SET status = $1, outstanding_balance = 0 WHERE id = $2 AND status = $3', ['paid_off', loanId, 'active']);
        }
        const err = new Error('Loan is already paid off.');
        err.statusCode = 400;
        throw err;
    }
    const recentPayment = await database_prod_1.default.get('SELECT id FROM loan_payments WHERE loan_id = $1 AND payment_date > NOW() - INTERVAL \'5 seconds\'', [loanId]);
    if (recentPayment) {
        const err = new Error('A payment was recently processed for this loan. Please wait a moment before trying again.');
        err.statusCode = 400;
        throw err;
    }
    const weeklyPayment = parseFloat(loan.weekly_payment || loan.monthly_payment / 4.33);
    ...
    if (paymentAmount < 0.01) {
        const err = new Error('Payment amount is too small. Loan may already be paid off.');
        err.statusCode = 400;
        throw err;
    }
    ...
    try {
        ...
        await client.query('UPDATE accounts SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [paymentAmount, account.id]);
        await client.query('INSERT INTO loan_payments (loan_id, amount, payment_date) VALUES ($1, $2, CURRENT_TIMESTAMP)', [loanId, paymentAmount]);
        if (isFinalPayment) {
            await client.query('UPDATE loans SET status = $1, outstanding_balance = 0 WHERE id = $2', ['paid_off', loanId]);
        }
        else {
            await client.query('UPDATE loans SET outstanding_balance = $1 WHERE id = $2', [clearedOutstanding, loanId]);
        }
        ...
        await client.query('INSERT INTO transactions (from_account_id, amount, transaction_type, description) VALUES ($1, $2, $3, $4)', [account.id, paymentAmount, 'loan_repayment', description]);
        await client.query('COMMIT');
    }
    catch (error) {
        await client.query('ROLLBACK');
...
router.post('/activate/:loan_id', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        ...
        try {
            ...
            await client.query('UPDATE loans SET status = $1, due_date = $2 WHERE id = $3', ['active', nextMonday.toISOString().split('T')[0], loan_id]);
            await client.query('COMMIT');
            res.json({
...
router.post('/admin/fix-approved', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        console.log('🔧 Fixing all approved loans...');
        const nextMonday = getNextMonday();
        // Get all approved loans
        const approvedLoans = await database_prod_1.default.query('SELECT * FROM loans WHERE status = $1', ['approved']);
        ...
        for (const loan of approvedLoans) {
            try {
                ...
                try {
                    ...
                    await client.query('UPDATE loans SET status = $1, due_date = $2 WHERE id = $3', ['active', nextMonday.toISOString().split('T')[0], loan.id]);
                    await client.query('COMMIT');
...
router.post('/admin/cleanup-zero-transactions', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        console.log('🧹 Cleaning up zero-amount loan repayment transactions...');
        // Find and delete zero-amount loan repayment transactions
        const result = await database_prod_1.default.query('DELETE FROM transactions WHERE transaction_type = $1 AND amount = $2', ['loan_repayment', '0.00']);
...
router.post('/admin/fix-outstanding-balances', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        console.log('🔧 Fixing outstanding balances for all loans...');
        // Get all active loans
        const loans = await database_prod_1.default.query('SELECT * FROM loans WHERE status = $1', ['active']);
        ...
        for (const loan of loans) {
            try {
                ...
                const totalPaidResult = await database_prod_1.default.get('SELECT COALESCE(SUM(amount), 0) as total FROM loan_payments WHERE loan_id = $1', [loan.id]);
                ...
                await database_prod_1.default.run('UPDATE loans SET outstanding_balance = $1 WHERE id = $2', [correctOutstandingBalance, loan.id]);
                results.push({
                    loan_id: loan.id,
                    status: 'success',
...
router.post('/admin/reset-all-loans', auth_1.authenticateToken, (0, auth_1.requireRole)(['teacher']), async (req, res) => {
    try {
        console.log('⚠️ RESETTING ALL LOAN DATA - This action cannot be undone!');
        // Get database connection for transaction
        const client = await database_prod_1.default.getClient();
        try {
            await client.query('BEGIN');
