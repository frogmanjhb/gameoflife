...
router.post('/apply', [
    ...
    try {
      ...
      ) VALUES ($1, $2, $3, $4, $5, $6, $7)`, [req.user.id, amount, termMonths, interestRate, 'pending', totalAmount, monthlyPayment]);
        res.status(201).json({
...
router.post('/pay', [
    ...
    try {
        ...
        const recentPayment = await database_prod_1.default.get('SELECT id FROM loan_payments WHERE loan_id = $1 AND payment_date > NOW() - INTERVAL \'5 seconds\'', [loan_id]);
        if (recentPayment) {
            return res.status(400).json({
        ...
        try {
            ...
            await client.query('UPDATE accounts SET balance = balance - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [actualPaymentAmount, account.id]);
            // Record loan payment
            await client.query('INSERT INTO loan_payments (loan_id, amount, payment_date) VALUES ($1, $2, CURRENT_TIMESTAMP)', [loan_id, actualPaymentAmount]);
            ...
            // Record transaction
            await client.query('INSERT INTO transactions (from_account_id, amount, transaction_type, description) VALUES ($1, $2, $3, $4)', [account.id, actualPaymentAmount, 'loan_repayment', `Loan payment - R${actualPaymentAmount.toFixed(2)}`]);
            await client.query('COMMIT');
