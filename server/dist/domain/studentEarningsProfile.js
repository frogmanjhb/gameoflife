...
exports.buildStudentEarningsProfile = buildStudentEarningsProfile;
...
function isTrackedEarningTransaction(description) {
    const d = description || '';
    return (/^Salary for/i.test(d) ||
...
async function buildStudentEarningsActivity(userId) {
    const xpItems = [];
    const moneyItems = [];
    const mathSessions = await database_prod_1.default.query(`SELECT id, difficulty, score, correct_answers, total_problems, earnings, played_at
     FROM math_game_sessions WHERE user_id = $1 ORDER BY played_at DESC LIMIT $2`, [userId, ACTIVITY_HISTORY_LIMIT]);
    ...
    const wordleSessions = await database_prod_1.default.query(`SELECT id, status, guesses_count, earnings, played_at
     FROM wordle_sessions
     WHERE user_id = $1 AND status IN ('won', 'lost')
     ORDER BY played_at DESC LIMIT $2`, [userId, ACTIVITY_HISTORY_LIMIT]);
    for (const row of wordleSessions) {
        ...
        if (xp > 0)
            xpItems.push({ ...base, xp });
        if (earnings > 0)
            moneyItems.push({ ...base, money: earnings });
    }
    const jobSessions = await database_prod_1.default.query(`SELECT * FROM (${JOB_GAME_SESSIONS_DETAIL_UNION}) s ORDER BY played_at DESC LIMIT $2`, [userId, ACTIVITY_HISTORY_LIMIT]);
    ...
    const taskTransactions = await database_prod_1.default.query(`SELECT t.id, t.amount, t.description, t.created_at
     FROM transactions t
     ...
     LIMIT $2`, [userId, ACTIVITY_HISTORY_LIMIT]);
    for (const row of taskTransactions) {
        const amount = parseAmount(row.amount);
        if (amount <= 0 || !isTrackedEarningTransaction(row.description))
            continue;
        const { source, label } = classifyTransaction(row.description);
        const detail = row.description === label || row.description === 'CLASS_EVENT_SUGGESTION_EARN' ||
    ...
    const sortNewest = (a, b) => new Date(b.occurred_at).getTime() - new Date(a.occurred_at).getTime();
    return {
...
async function buildStudentEarningsProfile(userId) {
    const user = await database_prod_1.default.get(`SELECT u.id, u.job_level, u.job_experience_points, j.name AS job_name, a.balance AS account_balance
     FROM users u
    ...
    if (!user)
        return null;
    const mathStats = await database_prod_1.default.get(`SELECT COUNT(*)::int AS sessions, COALESCE(SUM(earnings), 0)::numeric AS earnings
     FROM math_game_sessions WHERE user_id = $1`, [userId]);
    const wordleStats = await database_prod_1.default.get(`SELECT
     ...
     FROM wordle_sessions WHERE user_id = $1`, [userId]);
    const jobGameStats = await database_prod_1.default.get(`WITH job_sessions AS (${JOB_GAME_SESSIONS_UNION})
     SELECT COUNT(*)::int AS sessions,
            COALESCE(SUM(experience_points), 0)::int AS xp,
            COALESCE(SUM(earnings), 0)::numeric AS earnings
     FROM job_sessions`, [userId]);
    const taskMoney = await database_prod_1.default.get(`SELECT COALESCE(SUM(t.amount), 0)::numeric AS total
     FROM transactions t
     JOIN accounts a ON t.to_account_id = a.id
     WHERE a.user_id = $1
       AND t.transaction_type IN ('deposit', 'transfer')
    ...
    const salaryMoney = await database_prod_1.default.get(`SELECT COALESCE(SUM(t.amount), 0)::numeric AS total
     FROM transactions t
     JOIN accounts a ON t.to_account_id = a.id
     WHERE a.user_id = $1
       AND t.transaction_type IN ('deposit', 'transfer')
...
//# sourceMappingURL=studentEarningsProfile.js.map