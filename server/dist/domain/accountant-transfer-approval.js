...
exports.TRANSFER_APPROVAL_EARNINGS_REWARD = exports.TRANSFER_APPROVAL_XP_REWARD = void 0;
...
async function payTransferApprovalReward(client, userId, username, townClass, schoolId) {
    ...
    const newXP = currentXP + exports.TRANSFER_APPROVAL_XP_REWARD;
    ...
    if (!account || exports.TRANSFER_APPROVAL_EARNINGS_REWARD <= 0) {
        return {
    ...
    if (!townRow || treasuryBalance < exports.TRANSFER_APPROVAL_EARNINGS_REWARD) {
        throw new Error('TREASURY_INSUFFICIENT');
    }
    if (schoolId != null) {
        await client.query('UPDATE town_settings SET treasury_balance = treasury_balance - $1, updated_at = CURRENT_TIMESTAMP WHERE class = $2 AND school_id = $3', [exports.TRANSFER_APPROVAL_EARNINGS_REWARD, townClass, schoolId]);
    }
    else {
        await client.query('UPDATE town_settings SET treasury_balance = treasury_balance - $1, updated_at = CURRENT_TIMESTAMP WHERE class = $2 AND school_id IS NULL', [exports.TRANSFER_APPROVAL_EARNINGS_REWARD, townClass]);
    }
    await client.query('INSERT INTO treasury_transactions (school_id, town_class, amount, transaction_type, description, created_by) VALUES ($1, $2, $3, $4, $5, $6)', [
        schoolId,
        townClass,
        exports.TRANSFER_APPROVAL_EARNINGS_REWARD,
        'withdrawal',
        `Accountant transfer approval payout to ${username}`,
        userId,
    ]);
    await client.query('UPDATE accounts SET balance = balance + $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [exports.TRANSFER_APPROVAL_EARNINGS_REWARD, account.id]);
    await client.query(`INSERT INTO transactions (to_account_id, amount, transaction_type, description)
     VALUES ($1, $2, 'deposit', $3)`, [account.id, exports.TRANSFER_APPROVAL_EARNINGS_REWARD, 'ACCOUNTANT_TRANSFER_APPROVAL_EARN']);
    return {
        experience_points: exports.TRANSFER_APPROVAL_XP_REWARD,
        earnings: exports.TRANSFER_APPROVAL_EARNINGS_REWARD,
        new_level: newLevel > currentLevel ? newLevel : null,
    };
}
