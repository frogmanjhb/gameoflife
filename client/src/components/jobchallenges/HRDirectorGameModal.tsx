...
const HRDirectorGameModal: React.FC<HRDirectorGameModalProps> = ({
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await hrDirectorGameApi.startGame({ difficulty: diff });
          ...
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-400">
                {gameStatus?.remaining_plays ?? 0} / {gameStatus?.daily_limit ?? 3} HR review cycles remaining today
              </p>
            </div>
