...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, Users } from 'lucide-react';
import { hrDirectorGameApi } from '../../services/api';
import { HRDirectorGameStatus } from '../../types';
import { HRDirectorQuestion, getHRDirectorQuestion } from '../../utils/jobchallenges/hrDirectorQuestions';

interface HRDirectorGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: HRDirectorGameStatus | null;
  testMode?: boolean;
}
...
const HRDirectorGameModal: React.FC<HRDirectorGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<HRDirectorQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await hrDirectorGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getHRDirectorQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start HR review cycle. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getHRDirectorQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await hrDirectorGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit HR review cycle results'
      });
    } finally {
      ...
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-white">People Management Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete HR review cycles (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} HR review cycles remaining today`}
              </p>
            </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Attendance & Totals'}
                    {diff === 'medium' && 'Performance Analysis'}
                    {diff === 'hard' && 'Resource Allocation'}
                    {diff === 'extreme' && 'Workforce Strategy'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No HR review cycles remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">HR Review Cycle Complete!</h3>
              {gameResults.error && (
...
export default HRDirectorGameModal;
