...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, ShoppingBag } from 'lucide-react';
import { retailManagerGameApi } from '../../services/api';
import { RetailManagerGameStatus } from '../../types';
import { RetailManagerQuestion, getRetailManagerQuestion } from '../../utils/jobchallenges/retailManagerQuestions';

interface RetailManagerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
...
const RetailManagerGameModal: React.FC<RetailManagerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus
  ...
  const [currentProblem, setCurrentProblem] = useState<RetailManagerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      ...
      setCurrentProblem(getRetailManagerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start trading day review. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getRetailManagerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await retailManagerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit trading day review results'
      });
    } finally {
  ...
  const difficultyLabels: Record<Difficulty, string> = {
    easy: 'Sales & Totals',
    medium: 'Markup & Profit',
    hard: 'Discounts & Promotions',
    extreme: 'Supply & Demand'
  };

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-orange-500 to-emerald-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <ShoppingBag className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Shop Profit Challenge</h2>
          </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="flex items-center space-x-2 mb-4">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:border-orange-500 text-lg"
                    autoFocus
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-orange-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
                    onClick={handleAnswerSubmit}
                    disabled={!userAnswer.trim()}
                    className="w-full mt-4 bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 rounded-lg transition-colors disabled:bg-gray-700 disabled:text-gray-500"
                  >
                    Submit Answer
                  </button>
                )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Trading Day Review Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-orange-400 flex items-center space-x-1">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-emerald-400">R{gameResults.earnings.toFixed(2)}</span>
                </div>
                {gameResults.isNewHighScore && (
                  <div className="text-center text-orange-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default RetailManagerGameModal;
