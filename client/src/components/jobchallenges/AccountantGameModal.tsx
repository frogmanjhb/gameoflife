...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, Calculator } from 'lucide-react';
import { accountantGameApi } from '../../services/api';
import { AccountantGameStatus } from '../../types';
import { AccountantQuestion, getAccountantQuestion } from '../../utils/jobchallenges/accountantQuestions';

interface AccountantGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
...
const AccountantGameModal: React.FC<AccountantGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus
  ...
  const [currentProblem, setCurrentProblem] = useState<AccountantQuestion | null>(null);
  ...
  const MAX_PROBLEMS = 5; // 5 questions per audit case

  const spamMessages = [
    "One answer per question, please!",
    "Wait for the next question!",
    "Patience, auditor!",
    "No spam in the ledger!",
    "Slow down there!",
  ];
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      ...
      setCurrentProblem(getAccountantQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start audit case. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getAccountantQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await accountantGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit audit results'
      });
    } finally {
      ...
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-white">Financial Audit Challenges</h2>
          </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Income & Expenses'}
                    {diff === 'medium' && 'Gross vs Net & Tax'}
                    {diff === 'hard' && 'Budget Analysis'}
                    {diff === 'extreme' && 'Financial Strategy'}
                  </div>
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Audit Case Complete!</h3>
              {gameResults.error && (
...
export default AccountantGameModal;
