...
import { CivilEngineerQuestion, getCivilEngineerQuestion } from '../../utils/jobchallenges/civilEngineerQuestions';

interface CivilEngineerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: CivilEngineerGameStatus | null;
  testMode?: boolean;
}
...
const CivilEngineerGameModal: React.FC<CivilEngineerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<CivilEngineerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await civilEngineerGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getCivilEngineerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start infrastructure project. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getCivilEngineerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await civilEngineerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit infrastructure project results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-slate-600 to-stone-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Building2 className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Infrastructure Design Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete infrastructure projects (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} infrastructure projects remaining today`}
              </p>
            </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Measurement & Area'}
                    {diff === 'medium' && 'Cost per Metre'}
                    {diff === 'hard' && 'Load & Volume'}
                    {diff === 'extreme' && 'Terrain & Biome Adaptation'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No infrastructure projects remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
          ...
          <div className="p-6 space-y-4">
            ...
            {currentProblem && (
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-slate-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Infrastructure Project Complete!</h3>
              {gameResults.error && (
...
export default CivilEngineerGameModal;
