...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, Shield } from 'lucide-react';
import { insuranceManagerGameApi } from '../../services/api';
import { InsuranceManagerGameStatus } from '../../types';
import { InsuranceManagerQuestion, getInsuranceManagerQuestion } from '../../utils/jobchallenges/insuranceManagerQuestions';

interface InsuranceManagerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: InsuranceManagerGameStatus | null;
  testMode?: boolean;
}
...
const InsuranceManagerGameModal: React.FC<InsuranceManagerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<InsuranceManagerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await insuranceManagerGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getInsuranceManagerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start risk review cycle. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    const userAnswerNum = parseFloat(userAnswer.replace(/,/g, '').trim());
    const diff = Math.abs(userAnswerNum - currentProblem.answer);
    const correct = diff < 0.01 || (currentProblem.answer !== 0 && diff / Math.abs(currentProblem.answer) < 0.01);
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getInsuranceManagerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await insuranceManagerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit risk review cycle results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-violet-600 to-purple-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Shield className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Risk Review Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete risk review cycles (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} risk review cycles remaining today`}
              </p>
            </div>
                ...
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all ${
                    !testMode && (!gameStatus || (gameStatus.remaining_plays ?? 0) <= 0)
                      ...
                      : 'border-violet-500 bg-gray-800 text-white hover:bg-violet-600 hover:border-violet-400'
                  }`}
                >
                  <div className="text-lg font-semibold capitalize mb-1">{diff}</div>
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Premium Basics'}
                    {diff === 'medium' && 'Policy Pools'}
                    {diff === 'hard' && 'Claims & Ratios'}
                    {diff === 'extreme' && 'Town Risk Strategy'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No risk review cycles remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="flex items-center space-x-2 mb-4">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:border-violet-500 text-lg"
                    autoFocus
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-purple-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
                    onClick={handleAnswerSubmit}
                    disabled={!userAnswer.trim()}
                    className="w-full mt-4 bg-violet-600 hover:bg-violet-700 text-white font-semibold py-3 rounded-lg transition-colors disabled:bg-gray-700 disabled:text-gray-500"
                  >
                    Submit Answer
                  </button>
                )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              <Trophy className="h-16 w-16 text-violet-400 mx-auto mb-4" />
              <h3 className="text-2xl font-bold text-white mb-2">Risk Review Cycle Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-violet-400 flex items-center space-x-1">
                ...
                {gameResults.isNewHighScore && (
                  <div className="text-center text-violet-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default InsuranceManagerGameModal;
