...
import { EntrepreneurQuestion, getEntrepreneurQuestion } from '../../utils/jobchallenges/entrepreneurQuestions';

interface EntrepreneurGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: EntrepreneurGameStatus | null;
  testMode?: boolean;
}
...
const EntrepreneurGameModal: React.FC<EntrepreneurGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<EntrepreneurQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await entrepreneurGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getEntrepreneurQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start business scenario. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getEntrepreneurQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await entrepreneurGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit business scenario results'
      });
    } finally {
  ...
  const difficultyLabels: Record<Difficulty, string> = {
    easy: 'Profit Basics',
    medium: 'Break-even Thinking',
    hard: 'Growth & Percentage',
    extreme: 'Risk & Investment'
  };

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-violet-500 to-amber-500 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Rocket className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Business Builder Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete business scenarios (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} business scenarios remaining today`}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4">
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">{difficultyLabels[diff]}</div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No business scenarios remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Business Scenario Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-amber-400">R{gameResults.earnings.toFixed(2)}</span>
                </div>
...
export default EntrepreneurGameModal;
