...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, Activity } from 'lucide-react';
import { doctorGameApi } from '../../services/api';
import { DoctorGameStatus } from '../../types';
import { DoctorQuestion, getDoctorQuestion } from '../../utils/jobchallenges/doctorQuestions';

interface DoctorGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: DoctorGameStatus | null;
  testMode?: boolean;
}
...
const DoctorGameModal: React.FC<DoctorGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<DoctorQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await doctorGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getDoctorQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start health investigation. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getDoctorQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await doctorGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit health investigation results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-indigo-500 to-sky-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Activity className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Public Health & Biome Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete health investigations (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} health investigations remaining today`}
              </p>
            </div>
                ...
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all ${
                    !testMode && (!gameStatus || (gameStatus.remaining_plays ?? 0) <= 0)
                      ...
                      : 'border-indigo-500 bg-gray-800 text-white hover:bg-indigo-600 hover:border-indigo-400'
                  }`}
                >
                  <div className="text-lg font-semibold capitalize mb-1">{diff}</div>
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Common Illness Tracking'}
                    {diff === 'medium' && 'Spread & Growth'}
                    {diff === 'hard' && 'Resource Allocation'}
                    {diff === 'extreme' && 'Outbreak Modelling'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No health investigations remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="flex items-center space-x-2 mb-4">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:border-indigo-500 text-lg"
                    autoFocus
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-indigo-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
                    onClick={handleAnswerSubmit}
                    disabled={!userAnswer.trim()}
                    className="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-lg transition-colors disabled:bg-gray-700 disabled:text-gray-500"
                  >
                    Submit Answer
                  </button>
                )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              <Trophy className="h-16 w-16 text-indigo-400 mx-auto mb-4" />
              <h3 className="text-2xl font-bold text-white mb-2">Health Investigation Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-indigo-400 flex items-center space-x-1">
                ...
                {gameResults.isNewHighScore && (
                  <div className="text-center text-indigo-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default DoctorGameModal;
