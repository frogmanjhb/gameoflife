...
import { policeLieutenantGameApi } from '../../services/api';
import { PoliceLieutenantGameStatus } from '../../types';
import { PoliceLieutenantQuestion, getPoliceLieutenantQuestion } from '../../utils/jobchallenges/policeLieutenantQuestions';

interface PoliceLieutenantGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: PoliceLieutenantGameStatus | null;
  testMode?: boolean;
}
...
const PoliceLieutenantGameModal: React.FC<PoliceLieutenantGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<PoliceLieutenantQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await policeLieutenantGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getPoliceLieutenantQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start compliance review. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getPoliceLieutenantQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await policeLieutenantGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit compliance review results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-slate-600 to-blue-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Shield className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Enforcement Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete compliance reviews (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} compliance reviews remaining today`}
              </p>
            </div>
                ...
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all ${
                    !testMode && (!gameStatus || (gameStatus.remaining_plays ?? 0) <= 0)
                      ...
                      : 'border-slate-500 bg-gray-800 text-white hover:bg-slate-600 hover:border-slate-400'
                  }`}
                >
                  <div className="text-lg font-semibold capitalize mb-1">{diff}</div>
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Fines & Totals'}
                    {diff === 'medium' && 'Escalating Penalties'}
                    {diff === 'hard' && 'Compliance Analysis'}
                    {diff === 'extreme' && 'Policy Impact Modelling'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No compliance reviews remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="flex items-center space-x-2 mb-4">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:border-slate-500 text-lg"
                    autoFocus
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-blue-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
                    onClick={handleAnswerSubmit}
                    disabled={!userAnswer.trim()}
                    className="w-full mt-4 bg-slate-600 hover:bg-slate-700 text-white font-semibold py-3 rounded-lg transition-colors disabled:bg-gray-700 disabled:text-gray-500"
                  >
                    Submit Answer
                  </button>
                )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Compliance Review Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-slate-400 flex items-center space-x-1">
                ...
                {gameResults.isNewHighScore && (
                  <div className="text-center text-slate-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default PoliceLieutenantGameModal;
