...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, Map } from 'lucide-react';
import { townPlannerGameApi } from '../../services/api';
import { TownPlannerGameStatus } from '../../types';
import { TownPlannerQuestion, getTownPlannerQuestion } from '../../utils/jobchallenges/townPlannerQuestions';

interface TownPlannerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: TownPlannerGameStatus | null;
  testMode?: boolean;
}
...
const TownPlannerGameModal: React.FC<TownPlannerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<TownPlannerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await townPlannerGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getTownPlannerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start planning proposal. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getTownPlannerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await townPlannerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit planning proposal results'
      });
    } finally {
      ...
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-bold text-white">Zoning & Biome Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete planning proposals (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} planning proposals remaining today`}
              </p>
            </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Land Area & Cost'}
                    {diff === 'medium' && 'Zoning Percentages'}
                    {diff === 'hard' && 'Sustainability Constraints'}
                    {diff === 'extreme' && 'Infrastructure Modelling'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No planning proposals remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
          ...
          <div className="p-6 space-y-4">
            ...
            {currentProblem && (
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-emerald-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Planning Proposal Complete!</h3>
              {gameResults.error && (
...
export default TownPlannerGameModal;
