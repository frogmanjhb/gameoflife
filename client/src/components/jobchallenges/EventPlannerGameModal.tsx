...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, CalendarDays } from 'lucide-react';
import { eventPlannerGameApi } from '../../services/api';
import { EventPlannerGameStatus } from '../../types';
import { EventPlannerQuestion, getEventPlannerQuestion } from '../../utils/jobchallenges/eventPlannerQuestions';

interface EventPlannerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: EventPlannerGameStatus | null;
  testMode?: boolean;
}
...
const EventPlannerGameModal: React.FC<EventPlannerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<EventPlannerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await eventPlannerGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getEventPlannerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start event proposal. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getEventPlannerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await eventPlannerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit event proposal results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-amber-600 to-yellow-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <CalendarDays className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Event Budget Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Solve capacity, costs and pricing to complete event proposals</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} event proposals remaining today`}
              </p>
            </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Capacity & Basic Revenue'}
                    {diff === 'medium' && 'Costs & Profit'}
                    {diff === 'hard' && 'Pricing Strategy'}
                    {diff === 'extreme' && 'Time & Resource Management'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No event proposals remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Event Proposal Complete!</h3>
              {gameResults.error && (
...
export default EventPlannerGameModal;
