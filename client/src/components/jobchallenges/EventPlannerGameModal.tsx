...
const EventPlannerGameModal: React.FC<EventPlannerGameModalProps> = ({
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await eventPlannerGameApi.startGame({ difficulty: diff });
          ...
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-400">
                {gameStatus?.remaining_plays ?? 0} / {gameStatus?.daily_limit ?? 3} event proposals remaining today
              </p>
            </div>
