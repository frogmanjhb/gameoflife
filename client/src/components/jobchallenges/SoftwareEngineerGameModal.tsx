...
import { SoftwareEngineerQuestion, getSoftwareEngineerQuestion } from '../../utils/jobchallenges/softwareEngineerQuestions';

interface SoftwareEngineerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
...
const SoftwareEngineerGameModal: React.FC<SoftwareEngineerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus
  ...
  const [currentProblem, setCurrentProblem] = useState<SoftwareEngineerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      ...
      setCurrentProblem(getSoftwareEngineerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start build sprint. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getSoftwareEngineerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await softwareEngineerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit build sprint results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-blue-600 to-violet-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Code className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Logic & Systems Challenge</h2>
          </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Pattern & Sequence'}
                    {diff === 'medium' && 'Conditional Logic'}
                    {diff === 'hard' && 'Algorithmic Thinking'}
                    {diff === 'extreme' && 'Systems & Performance'}
                  </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                {!showFeedback && (
                  <button
                    ...
                    className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-lg transition-colors disabled:bg-gray-700 disabled:text-gray-500"
                  >
                    Submit Answer
                  </button>
                )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Build Sprint Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                ...
                {gameResults.isNewHighScore && (
                  <div className="text-center text-blue-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default SoftwareEngineerGameModal;
