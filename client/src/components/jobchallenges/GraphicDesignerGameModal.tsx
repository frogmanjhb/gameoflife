...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award, Palette } from 'lucide-react';
import { graphicDesignerGameApi } from '../../services/api';
import { GraphicDesignerGameStatus } from '../../types';
import { GraphicDesignerQuestion, getGraphicDesignerQuestion } from '../../utils/jobchallenges/graphicDesignerQuestions';

interface GraphicDesignerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
...
const GraphicDesignerGameModal: React.FC<GraphicDesignerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus
  ...
  const [currentProblem, setCurrentProblem] = useState<GraphicDesignerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      ...
      setCurrentProblem(getGraphicDesignerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start design brief. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getGraphicDesignerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await graphicDesignerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit design brief results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-teal-600 to-cyan-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Palette className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Design Precision Challenge</h2>
          </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Size & Dimensions'}
                    {diff === 'medium' && 'Ratios & Proportion'}
                    {diff === 'hard' && 'Layout & Spacing'}
                    {diff === 'extreme' && 'Scaling & Print'}
                  </div>
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="flex items-center space-x-2 mb-4">
                  <input
                    ...
                    className="flex-1 px-4 py-3 bg-gray-700 text-white rounded-lg border border-gray-600 focus:outline-none focus:border-teal-500 text-lg"
                    autoFocus
                ...
                {!showFeedback && (
                  <button
                    ...
                    className="w-full mt-4 bg-teal-600 hover:bg-teal-700 text-white font-semibold py-3 rounded-lg transition-colors disabled:bg-gray-700 disabled:text-gray-500"
                  >
                    Submit Answer
                  </button>
                )}
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              <Trophy className="h-16 w-16 text-teal-400 mx-auto mb-4" />
              <h3 className="text-2xl font-bold text-white mb-2">Design Brief Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-teal-400 flex items-center space-x-1">
                ...
                {gameResults.isNewHighScore && (
                  <div className="text-center text-teal-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default GraphicDesignerGameModal;
