...
import { X, Play, Trophy, Clock, Target, Zap, CheckCircle, XCircle, Award } from 'lucide-react';
import { electricalEngineerGameApi } from '../../services/api';
import { ElectricalEngineerGameStatus } from '../../types';
import { ElectricalEngineerQuestion, getElectricalEngineerQuestion } from '../../utils/jobchallenges/electricalEngineerQuestions';

interface ElectricalEngineerGameModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGameComplete: () => void;
  gameStatus: ElectricalEngineerGameStatus | null;
  testMode?: boolean;
}
...
const ElectricalEngineerGameModal: React.FC<ElectricalEngineerGameModalProps> = ({
  isOpen,
  onClose,
  onGameComplete,
  gameStatus,
  ...
  const [currentProblem, setCurrentProblem] = useState<ElectricalEngineerQuestion | null>(null);
  ...
  const startGame = async (diff: Difficulty) => {
    try {
      setDifficulty(diff);
      const response = await electricalEngineerGameApi.startGame({ difficulty: diff, ...(testMode ? { test: true } : {}) });
      ...
      setCurrentProblem(getElectricalEngineerQuestion(diff));
    } catch (error: any) {
      console.error('Failed to start game:', error);
      alert(error.response?.data?.error || 'Failed to start power allocation review. Please try again.');
    }
  };
  ...
  const handleAnswerSubmit = () => {
    ...
    setTimeout(() => {
      setUserAnswer('');
      setCurrentProblem(getElectricalEngineerQuestion(difficulty));
      setIsCorrect(null);
  ...
  const endGame = useCallback(async () => {
    ...
    try {
      const response = await electricalEngineerGameApi.submitGame({
      ...
      setGameResults({
        ...
        accuracy: answerSequence.length > 0 ? (score / answerSequence.length) * 100 : 0,
        error: error.response?.data?.error || 'Failed to submit power allocation review results'
      });
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        <div className="bg-gradient-to-r from-amber-500 to-yellow-600 p-6 rounded-t-2xl flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <Zap className="h-6 w-6 text-white" />
            <h2 className="text-xl font-bold text-white">Power Systems Challenge</h2>
          </div>
        ...
        {gameState === 'difficulty' && (
          <div className="p-6 space-y-4">
            <div className="text-center mb-6">
              <p className="text-gray-300 mb-2">Complete power allocation reviews (5 problems each) to earn XP and money</p>
              <p className="text-sm text-gray-400">
                {testMode ? 'Test mode – no limit' : `${gameStatus?.remaining_plays ?? 0} / ${gameStatus?.daily_limit ?? 3} power allocation reviews remaining today`}
              </p>
            </div>
              ...
              {(['easy', 'medium', 'hard', 'extreme'] as Difficulty[]).map((diff) => (
                ...
                >
                  ...
                  <div className="text-xs text-gray-400">
                    {diff === 'easy' && 'Power & Basic Calculations'}
                    {diff === 'medium' && 'Electricity Cost'}
                    {diff === 'hard' && 'Load Management'}
                    {diff === 'extreme' && 'Renewable Planning & Efficiency'}
                  </div>
            ...
            {!testMode && gameStatus && (gameStatus.remaining_plays ?? 0) <= 0 && (
              <div className="text-center text-gray-400 text-sm mt-4">
                No power allocation reviews remaining today. Try again tomorrow!
              </div>
            )}
          </div>
        )}
          ...
          <div className="p-6 space-y-4">
            ...
            {currentProblem && (
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                {spamMessage && (
                  <div className="mt-2 text-center text-amber-400 text-sm animate-pulse">{spamMessage}</div>
                )}
                {!showFeedback && (
                  <button
        ...
        {gameState === 'results' && gameResults && (
          <div className="p-6 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Power Allocation Review Complete!</h3>
              {gameResults.error && (
            ...
            <div className="bg-gray-800 rounded-lg p-6 space-y-4">
              ...
              <div className="border-t border-gray-700 pt-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-amber-400 flex items-center space-x-1">
                ...
                {gameResults.isNewHighScore && (
                  <div className="text-center text-amber-400 font-semibold mt-2">🏆 New High Score!</div>
                )}
              </div>
            </div>
            <button
...
export default ElectricalEngineerGameModal;
